...
マップ演算子の型は，
$\mFuncTypeConstructor{r}\mA=\mTypeAssemble{f}{\mA}$ とすると
\begin{equation}
  (\mMap)
...
\begin{equation}
  f\mBindComp g=f\mBind(g\mAnonParam)
\end{equation}
...
\begin{align}
  ...
  (\mMap)
  \mIn{}(\mA\mMapsTo\mB)
  \mMapsTo\mMaybeType{a}
  \mMapsTo\mMaybeType{b}\\
  \mInstancePolymorphic\;
  &\mFunctorTypeClass\mSuperSet\mMaybeTypeConstructor
  \mWhere\left\{
  \begin{aligned}
...
\begin{equation}
  \mTypeAssemble{f}{a}
  =\mathop{\mPolymorphicTypeParameter{f}}\mA
\end{equation}
...
$\mNewDataTypePolymorphic$ に置き換えたことである．これは，字面に反し
...
このような問題を解決するのが\keyword{圏}すなわち $\mCategoryTypeClass$
型クラスである．
...
$\mIdCat=\mId$ かつ $\mCompCat=\mComp$ になるので，圏は関数を何やら拡張
したものであることがわかる．
...
\newcommand{\mCircuitType}{\mType{Circuit}}
\newcommand{\mCircuitWith}[1]{\mValueWith{Circuit}{#1}}
\newcommand{\mFirstFunc}{\mathop{\mathrm{first}}}

\begin{equation}
  \mNewDataType\;
  \mCircuitType\,\mA\,\mB
  =\mTypeAssemble{Circuit}{\mUncircuit\mIn\mA\mMapsTo(\mCircuitType\,\mA\,\mB,\mB)}
\end{equation}
...
\begin{multline}
  ...
  \begin{aligned}
    ...
    \mCircuitWith{c_2}\mCompCat\mCircuitWith{c_1}
    &={}^{\textrm{Circuit}}\[\mLambda x\mLambdaArrow(c_2'\mComp c_1',z)\\
    &\quad\mWhere{}\{
    (c_1',y)\mLetEq c_1x;\\
    &\qquad(c_2',z)\mLetEq c_2y
    \}\]
  \end{aligned}
  \right.
\end{multline}
...
\begin{multline}
  ...
  \begin{aligned}
    \mArrowWith{f}
    &=\mCircuitWith{\mLambda x\mLambdaArrow(\mArrowWith{f},fx)}\\
    \mFirstFunc\mCircuitWith{c}
    &={}^{\mathrm{Circuit}}\[\mLambda xy\mLambdaArrow(\mFirstFunc c',(z,y))\\
    &\quad\mWhere{}(c',z)=cx\] % cz???
  \end{aligned}
  \right.
\end{multline}
...
\begin{align}
  ...
  &=\mLambda x\mLambdaArrow(\mArrowWith{\mConst x}\Rrightarrow\mTotal\bigvee\mArrowWith{\mConst1}\Rrightarrow\mTotal)...?%\Rrightarrow\mArrowWith{\mUncurry(/)}
\end{align}
...
\begin{equation}
  ...
  =(\mJustWith{\dotsb}\mComp(*2))\mBindComp(\mJustWith{\dotsb}\mComp(*3))
\end{equation}

\begin{multline}
  \mLambda x\mLambdaArrow((\mLambda y\mLambdaArrow\mJustWith{y*x})\mBind{}\mJustWith{x*2})\\
  =(\mJustWith{\dotsb}\mComp\mUncurry(*))\mBindComp(\mJustWith{\dotsb}\mComp{}((*2),\mId))
\end{multline}

---

\begin{equation}
  (f*\!*\!*g)\mPairWith{x}{y}=\mPairWith{fx}{gy}
\end{equation}

\begin{equation}
  (f\&\&\&g)x=\mPairWith{fx}{gx}
\end{equation}
