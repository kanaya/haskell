...
複数引数をとる関数を我々はよく$f'(x,y)$と書くが，これも括弧が冗長なのでいま後は$f'xy$と書くことにする．
...
関数にも型がある．
いまは深入りしないが，例えば整数引数を一つ取り，整数を返す関数は$$\integertype\rightarrow\integertype$$という型を持つ．
...
関数合成もまた関手の例である．
いま型$\typename{a}$を型$\function{\typename{a}}$に変換する関手$$\functionfunctor\typename{a}=\typename{a}\mapsto\function{\typename{a}}$$を考える．
...
ここで$\function{\typename{a}}$の定義式を用いると
\begin{align*}
