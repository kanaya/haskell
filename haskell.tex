...
タプル&丸括弧で包む&$\hxTupple{\hVar{x}}{\hVar{y}}$\\
有名な変数&ローマン&$\hSpecialVar{first}$\\
...
有名な関数&ローマン&$\hId$\\
関手（数学）&フラクチュール&$\hCatId$\\
\hline
アクション&ギリシア文字（1文字）&$\hAction{\alpha}$\\
有名なアクション&スラント&$\hMain$\\
\hline
型&ボールド（1文字）&$\hTypeName{a}$\\
型コンストラクタ&ボールド（1文字）&$\hTypeName{m}$\\
有名な型&ボールド大文字&$\hBool$\\
型クラス&スモールキャピタル&$\hEq$\\
% 型コンストラクタ&ボールドイタリック&$\hTypeConstructor{m},\hTypeConstructor{n}$\\
% 有名な型コンストラクタ&ボールド大文字&$\hList,\hMaybe$\\
多相型&型コンストラクタ名と型名&$\hxTypeConstructor{\hTypeName{m}}{\hTypeName{a}}$\\
有名な多相型&特別な括弧で包む&$\hxListConstructor{\hTypeName{a}}$\\
集合（数学）&ブラックボード大文字&$\hSet{B}$\\
\hline
\end{tabular}
\end{center}
\end{table*}
...
$\hxAnonParam$&無名パラメタ&\\
$(\hxAnonParam+\hxAnonParam)\,\hVar{x}\,\hVar{y}$&関数化&\code{(+) x y}\\
