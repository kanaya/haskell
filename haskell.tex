...
\item グローバル変数にゼロ除算エラーが起こったことを記録しておき，$x$ にはとりあえずの数値，例えば $\mZeroNumber$ を代入しておいて，計算を続行させる
...
ここで，引数に $\mOneNumber$ を加えて返す関数 \code{f} があるとしよう．関数 \code{f} の定義は次の通りである．
\begin{pythoncode}
\begin{verbatim}
f = lambda \mXVar: 1+x
\end{verbatim}
...
リストバージョンのアプリカティブマップ演算子 $(\mAppMapList)$ の特別な場合として，左引数のリストの要素数が $\mOneNumber$ の場合を考えると
\begin{equation}
