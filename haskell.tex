...
残念なことに関数 $\mSum$ も $\mLength$ も同じ引数を $(\mList{x})$ とる
必要がある．そのため従来の方法では関数を合成出来ない．しかし次のように
演算の結果をペアに収めることが出来れば何とかなりそうである．
\begin{equation}
  \mMean\mList{x}
  =%\left\{\begin{matrix}\mAnonParam\\/\\\mAnonParam\end{matrix}\right\}
  \left(\mLambda\mPairWith{x}{y}\mLambdaArrow x/y\right)
  \begin{pmatrix}
    \mSum\mList{x}\\
    \mLength\mList{x}
  \end{pmatrix}
  %\begin{bmatrix}\mSum\\\mLength\end{bmatrix}
  %\begin{pmatrix}\mList{x}\\\mList{x}\end{pmatrix}  
\end{equation}
...
アローとは関数の拡張で，関数の型を $\mB\mFuncArrow\mC$ と表すとき，対
応するアローの型を $\mB\mArrowArrow{\mPolymorphicTypeParameter{a}}\mC$
と表す．ここに $\mPolymorphicTypeParameter{a}$ は型コンストラクタであ
り，型クラス $\mArrowTypeClass$ のインスタンスである．
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    &\mSplit'
    =\mArrowWith{\mLambda x\mLambdaArrow\mPairWith{x}{x}}
  \end{aligned}
  \right.
\end{equation}
