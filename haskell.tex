...
リスト変数&チルダをつける&$\hListDecor{\hVar{x}}$\\ %%% !!!
Maybe変数&ドットをつける&$\hMaybeDecor{\hVar{x}}$\\ %%% !!!
Ether変数&ダブルドットをつける&$\hEitherDecor{\hVar{x}}$\\ %%% !!!
一般のコンテナ変数&ハットをつける&$\hAnyContextDecor{\hVar{x}}$\\
...
有名なアクション&ローマン&$\hSpecialAction{main},\hSpecialAction{print}$\\ %%% !!!
\hline
型&ボールド（1文字）&$\hTypeName{a},\hTypeName{b},\hTypeName{c}$\\
有名な型&ボールド大文字&$\hBool,\hInt$\\
型クラス&ボールドイタリック大文字&$\hEq,\hOrd$\\
...
集合&ブラックボード大文字&$\hSet{B},\hSet{Z},\hSet{R}$\\
\hline
...
% アンボックス化タプル&丸括弧とシャープ記号で包む&$\mTupleUnboxedWith{x,\hVar{y},\mZVar}$\\
\hline
\end{tabular}
\end{center}
\end{table*}
...
\hline
% $\hCompose$&関数合成&\code{.}\\
% $\hMonadicCompose$&モナド関数の合成&\code{<=<}\\
% $\hCategoryCompose$&圏の関数合成&\code{.}\\
% $\hApply$&関数適用&\code{\$}\\
% % $\mApplyRight$&右関数右適用&\\
% \hline
...
\end{equation}
と書いても良い．式を左から読んで1番目の $\hAnonParam$ が元々の $\hVar{x}$ すなわち第1引数を，2番目の $\hAnonParam$ が元々の $\hVar{y}$ すなわち第2引数を意味する．この省略記法はプログラミング言語\scheme における\code{cut}プロシジャに由来する．\footnote{この記法は\haskell にはない．}
...
\begin{equation}
  ...
  =(\hSin\hVar{x})/\hVar{x}
\end{equation}
と書いても良い．この書き方を\keyword{ポイントフリースタイル}と呼ぶ．ポイントフリースタイルは今後頻出するので，是非覚えておいてもらいたい．\footnote{\haskell では $\hFunc{f}=\hLambdaSyntax{\hVar{x}}{\hSin\hVar{x})/\hVar{x}}$ を \code{f = \textbackslash x -> (sin x)/x} と書き，一方 $\hFunc{f}\hVar{x}=(\hSin\hVar{x})/\hVar{x}$ を\code{f x = (sin x)/x} と書く．}
...
二項演算子のうちよく使われるものは和 $(+)$，積 $(*)$，論理和 $(\mLogicalOr)$，論理積 $(\mLogicalAnd)$，同値 $(\hIfEq)$，大なり$(>)$，小なり $(<)$ 等である．二項演算子はたとえ積記号であっても省略できない．二項演算子は多数あるので，その都度説明する．\footnote{\haskell では $\mLogicalAnd$ を \code{\&\&} と書き，$\mLogicalOr$ を \code{||} と書く．}

二項演算子は\keyword{中置}することが基本であるが，括弧で包むことで前置することも可能である．任意の二項演算子 $\mBinOp$ について $\hVar{x}\mBinOp\hVar{y}$ 及び $(\mBinOp)\hVar{x}\hVar{y}$ は全く同じ意味である．すなわち
\begin{equation}
  \left(\mBinOp\right)\hVar{x}\hVar{y}
  =\hVar{x}\mBinOp\hVar{y}
\end{equation}
である．従って，二項演算子と2引数関数に本質的な差はない．本書では演算子と関数という用語は全く同じ意味で用いる．\footnote{\haskell では任意の二項演算子を括弧で包むことで前置演算子として使うことができる．例えば \code{\hVar{x}+\hVar{y}}と \code{(+)x y} は同じ結果を返す．逆に任意の2引数関数 \code{f}は \code{x `f` y} と書くことで中置することができる．}

一般の関数が左結合であることを思い出すと，二項演算子を関数に見立てた
$(\mBinOp)$ も
\begin{equation}
  \label{eq:star-x-y}
  (\mBinOp)\hVar{x}\hVar{y}
  =((\mBinOp)\hVar{x})\hVar{y}
\end{equation}
であるから，部分適用が可能である．式\eqref{eq:star-x-y}から第2引数 $\hVar{y}$ を取り除いて $(\mBinOp)x$ という「餓えた」1引数関数を取り出せる．例えば関数 $((+)1)$ は引数に $\hConstant{1}$ を加える関数である．\footnote{\haskell では $((+)1)$ を \code{((+)1)} と書く．}

\separator

二項演算子の部分適用に限って\keyword{セクション}と言う記法も用いられる．二項演算子 $\mBinOp$ に対して $(\mBinOp\hVar{x})$ および $(\hVar{x}\mBinOp)$ はそれぞれ
\begin{align}
  (\mBinOp\hVar{x})
  &=\hAnonParam\mBinOp\hVar{x}\\
  (\hVar{x}\mBinOp)
  &=\hVar{x}\mBinOp\hAnonParam=(\mBinOp)\hVar{x}
\end{align}
である．例えば $(\hConstant{1}+)$ は $((+)\hConstant{1})$ と等価であり，これは $(+\hConstant{1})$ とも等価である．ただし，マイナス演算子$(-)$ だけは例外で，$(-\hConstant{1})$ はマイナス $\hConstant{1}$ を表す．負の数をいつも括弧で包んでおくのは良いアイディアである．\footnote{\haskell では $(1+)$ を\code{(1+)} と書く．また \code{(-1)} はセクションではなくマイナス$\hConstant{1}$を表す（\code{-1} というリテラルとみなされる）．ただし \code{(- 1)}のように空白を挟んでも同じくマイナス $\hConstant{1}$ とみなされる（\code{1} というリテラルに単項マイナス演算子が適用される）．}
...
\end{equation}
である．式\eqref{eq:ffx}から括弧を省略して $\mZVar=\hFunc{g}\hFunc{f}\hVar{x}$ としてしまうと，関数適用は左結合するから $\mZVar=(\hFunc{g}\hFunc{f})\hVar{x}$ の意味になってしまう．関数 $\hFunc{g}$ が引数に関数を取るのでない限り $(\hFunc{g}\hFunc{f})$ は無意味なので，式\eqref{eq:ffx}の括弧は省略できない．

ここで，引数のことは忘れて，関数 $\hFunc{f}$ と関数 $\hFunc{g}$ を先に\keyword{合成}しておきたいとしよう．その合成を $\hFunc{g}\mCompFunc \hFunc{f}$ と書く．演算子 $\mCompFunc$ は\keyword{関数合成演算子}と呼ぶ．合成はラムダ式を使って
\begin{equation}
  \hFunc{g}\mCompFunc \hFunc{f}
  =\hFunc{g}(\hFunc{f}\hAnonParam)
\end{equation}
と定義できる．関数合成演算子 $\mCompFunc$ は関数適用よりも優先順位が高く， $\left(\hFunc{g}\mCompFunc \hFunc{f}\right)\hVar{x}$ は単に $\hFunc{g}\mCompFunc\hFunc{f}\hVar{x}$ と書いても良い．この記法は括弧の数を減らすためにしばしば用いられる．式\eqref{eq:ffx}は関数合成演算子を用いると
\begin{equation}
  \mZVar
  =\hFunc{g}\mCompFunc\hFunc{f}\hVar{x}
\end{equation}
と書ける．\footnote{\haskell では関数 \code{g} と関数 \code{f} の合成は \code{g.f} である．式 $\mZVar=\hFunc{g}\mCompFunc \hFunc{f} x$ は \code{z = g.f x} と書く．}

関数合成演算子とは逆に，結合の優先順位の低い\keyword{関数適用演算子}も考えておくと便利なこともある．関数適用演算子 $\mApply$ を次のように定義しておく．
\begin{equation}
  \hFunc{f}\mApply\hVar{x}
  =\hFunc{f}\hVar{x}
\end{equation}
演算子 $\mApply$ の優先順位は関数適用も含めあらゆる演算子よりも低いものとする．関数適用演算子を用いて式\eqref{eq:ffx}を書き直すと
\begin{equation}
  \mZVar
  =\hFunc{g}\mApply\hFunc{f}\hVar{x}
\end{equation}
となる．演算子 $\mApply$ の優先順位は足し算よりも低いので $\hFunc{f}(\hVar{x}+\hConstant{1})$ は $\hFunc{f}\mApply\hVar{x}+\hConstant{1}$ と書くこともできる．演算子 $\mApply$ を閉じ括弧のいらない開き括弧と考えてもよい．\footnote{\haskell では $\hFunc{g}\mApply\hFunc{f}\hVar{x}$ を \code{g \$ f x} と書く．}

関数適用演算子のもう一つの興味深い使い方は，関数適用演算子の部分適用である．セクション $(\mApply\hVar{x})$ を用いると
\begin{equation}
  (\mApply\hVar{x})\hFunc{f}
  =\hFunc{f}\mApply\hVar{x}
\end{equation}
であるから，関数適用演算子を用いて引数を関数に渡すことができる．\footnote{\haskell では $(\mApply\hVar{x})\hFunc{f}$ を \code{(\$x)f} と書く．}

\section{高階関数}

関数を引数に取ったり，あるいは関数を返す関数のことを\keyword{高階関数}と呼ぶことがある．関数合成演算子と関数適用演算子は高階関数の好例である．

他に例えば，引数として整数 $\mAVar$ を取り，関数 $\hFunc{f}\hVar{x}=\mAVar+\hVar{x}$ を返すような関数 $\hFunc{g}$ を
\begin{equation}
  \hFunc{g}\mAVar
  =\mAVar+\hAnonParam
\end{equation}
...
\begin{align}
  \hFunc{f}
  &=\hFunc{g}\,\mConstant{100}\\
  \hVar{x}
  &=\hFunc{f}\,\hConstant{1}
\end{align}
とすれば $\hVar{x}=\mConstant{101}$ を得る．\footnote{\haskell では $ga=a+\hAnonParam$ を $ga=\mLambdaEXP{x}{a+x}$ と展開しておいて \code{g a = \textbackslash x -> a+x} と書く．}
...
\end{equation}
という意味である．\footnote{\haskell では $\hVar{x}\uparrow\hVar{y}$ を \code{x\textasciicircum y} と書く．}
...
\begin{align}
  \hVar{x}\uparrow\uparrow\mNVar
  &\hGuard{\mNVar\le\hConstant{0}}=\hConstant{1}\\
  &\hGuard{\hOtherwise}
  =\hVar{x}\uparrow(\hVar{x}\uparrow\uparrow(\mNVar-\hConstant{1}))
\end{align}
...
\begin{enumerate}
\item 関数と演算子は同じものである．
\item 任意の二項演算子 $\mBinOp$ について $\hVar{x}\mBinOp y$ と $(\mBinOp)\hVar{x}\hVar{y}$ は全く同じ意味である．
\item 任意の二項演算子 $\mBinOp$ について $(\mBinOp x)=\hAnonParam\mBinOp x$ であり，$(x\mBinOp)=x\mBinOp\hAnonParam$ である．これらの記法をセクション記法と呼ぶ．
\item 関数 $\hFunc{f}$ と関数 $\hFunc{g}$ を合成した関数を $f\mCompFunc g$ と書く．
\item 式 $f\mApply \hFunc{g}\hVar{x}$ は $f(\hFunc{g}\hVar{x})$ の意味である．
...
この場合変数 $\hVar{x}$ が $\mBoolType$ 型であることは自明であるため $\hVar{x}\mIn\mBoolType$ は省略できるが，可能な限り型を明記しておくことは良い習慣である．また必要に応じて変数に型名を注釈することがある．例えば $\hVar{x}=\hTrue\mIn\mBoolType$ のように書く場合がある．\footnote{\haskell では $\hVar{x}=\hTrue\mIn\mBoolType$ を \code{x = True :: Bool} と書く．}
...
\end{equation}
のようにイメージすると良い．これは関数 $\hFunc{f}$ が集合 $\mIntType$ から集合 $\mIntType$ への\keyword{写像}であると読む．\footnote{\haskell では \code{f :: Int -> Int} と書く．}
...
\end{equation}
はタプルである．変数 $\hVar{x}$, $\hVar{y}$ の型は同じでも良いし，異なっても良い．\footnote{\haskell では $\mPairWith{\hVar{x}}{\hVar{y}}$ を \code{(x, y)} と書く．}
...
他に\keyword{論理集合} $\mBSet=\{\hTrue,\hFalse\}$ に対して，\keyword{論理和} $(\mLogicalOr)$ は全域性を持つから，組み合わせ $\mTupleWith{\mBSet,\mLogicalOr}$ はマグマであるし，同様に\keyword{論理積} $(\mLogicalAnd)$ も全域性を持つから，組み合わせ $\mTupleWith{\mBSet,\mLogicalAnd}$ もマグマである．論理集合 $\mBSet$ とは論理型 $\mBoolType$ を数学風に言い換えたものである．

マグマのうち，演算を2回続ける場合，その順序によって結果が異ならない，つまり
\begin{equation}
  \label{eq:associativity}
  \left(\mAVar_1\mBinOp \mAVar_2\right)\mBinOp \mAVar_3
  =\mAVar_1\mBinOp\left(\mAVar_2\mBinOp{\mAVar_3}\right)
\end{equation}
ただし $\mAVar_1,\mAVar_2,\mAVar_3\mIn\mASet$ のとき，組み合わせ $\mTupleWith{\mASet,\mBinOp}$ のことを\keyword{半群}と呼ぶ．この式\eqref{eq:associativity}で表される性質を\keyword{結合性}と呼ぶ．組み合わせ $\mTupleWith{\mZSet,+}$, $\mTupleWith{\mZSet,*}$, $\mTupleWith{\mBSet,\mLogicalOr}$, $\mTupleWith{\mBSet,\mLogicalOr}$ はすべて半群である．

ところで，整数全体の集合 $\mZSet$ には特別な元 $\hConstant{0}\mIn\mZSet$ がある．この元 $\hConstant{0}$ は $\mZVar\mIn\mZSet$ のとき
\begin{equation}
  \hConstant{0}+\mZVar
  =\mZVar+\hConstant{0}
  =\mZVar
...
一般に，$\mAVar,\mZeroLeft,\mZeroRight\in\mASet$ として
\begin{equation}
  \label{eq:identity}
  \mZeroLeft\mPlus\mAVar
  =\mAVar\mPlus\mZeroRight
  =\mAVar
\end{equation}
...
組み合わせ $\mTupleWith{\mASet,\mPlus,\mZeroLeft,\mZeroRight}$ のことを\keyword{モノイド}または\keyword{単位的半群}と呼ぶ．例えば $\mTupleWith{\mZSet,+,\hConstant{0},\hConstant{0}}$ はモノイドであるし，$\mTupleWith{\mZSet,*,\hConstant{1},\hConstant{1}}$, $\mTupleWith{\mBSet,\mLogicalOr,\hFalse,\hFalse}$, $\mTupleWith{\mBSet,\mLogicalAnd,\hTrue,\hTrue}$ もモノイドである．これらのモノイドは全て左単位元と右単位元が同じなので，それぞれ $\mTupleWith{\mZSet,+,\hConstant{0}}$, $\mTupleWith{\mZSet,*,\hConstant{1}}$, $\mTupleWith{\mBSet,\mLogicalOr,\hFalse}$, $\mTupleWith{\mBSet,\mLogicalAnd,\hTrue}$ とも書く．
...
我々も $\hConstant{0}$ から始まり $\mNineNumber$ まで続く整数のリストを $[\hConstant{1},\mTwoNumber,\mThreeNumber,\mFourNumber,\mFiveNumber,\mSixNumber,\mSevenNumber,\mEightNumber,\mNineNumber]$ と書くことにしよう．ただし，これでは冗長なので\keyword{等差数列}に限って簡略化した書き方を許す．例えば $\hConstant{0}$ から $\mNineNumber$ までのリストは $[\hConstant{0},\hConstant{1}\dotsb\mNineNumber]$ と書いても良い．\footnote{\haskell では \code{[0, 1..9]} と書く．ピリオドの数に注意しよう．}
...
\end{equation}
のように書くことにしよう．なお，変数 $\hVar{x}$ とリスト変数 $\mList{x}$ は異なる変数であるとする．\footnote{\haskell では \code{s} を変数名にくっつけて \code{xs = [0, 1..9]} のように書く習慣がある．}
...
\end{equation}
のように書こう．ここに右辺のリストから一つずつ要素を取り出して左辺に代入する演算子$\mFrom$を用いた．\footnote{\haskell では $\mList{x}=[\hVar{x}*2\mListComp\hVar{x}\mFrom[\hConstant{0},\hConstant{1}\dotsb\mNineNumber]]$ を \code{xs = [x*2 | x<-[0,1..9]]} と書く．}
...
\end{verbatim}
\end{pythoncode}
のことである．\footnote{\haskell では \code{xs = [x+y | x<-[\hConstant{0},\hConstant{1}..9], y<-[\hConstant{0},\hConstant{1}..5], x+y>\mThreeNumber]} と書く．}
...
$[\hVar{x}]$ のように $\mA$ 型の変数 $\hVar{x}$ を入れた$[\mA]$ 型の変数を作る演算子を\keyword{リスト値コンストラクタ}と呼ぶ．$[\mA]$ 型の変数のことを\keyword{リスト変数}とも呼ぶ．$\mA$ 型の変数 $\hVar{x}$ からリスト値コンストラクタを使ってリスト$\mList{x}$ を作ることは
\begin{equation}
  \mList{x}
  =[\hVar{x}]
\end{equation}
...
\end{equation*}
とすると，リスト $\mList{x}$ は $\mList{x}=[\hConstant{1},\mTwoNumber\dotsb\mFiveNumber]$ という値を持つ．\footnote{\haskell では \code{xs = take 5 ns} と書く．}
...
\end{equation}
で定義される演算子である．この表現を一般化してみよう．リスト $[\hVar{x}_0,\hVar{x}_1\dotsb \hVar{x}_n]$ が与えられたとき，任意の二項演算子を $\mBinOp$ として
\begin{equation}
  \mFold^{\mBinOp}_a[\hVar{x}_0,\hVar{x}_1\dotsb \hVar{x}_n]
  =a\mBinOp \hVar{x}_0\mBinOp \hVar{x}_1\mBinOp\dotsb\mBinOp \hVar{x}_n
\end{equation}
であると定義する．
...
\begin{equation}
  \mFoldRight^{\mBinOp}_\mAVar[\hVar{x}_0,\hVar{x}_1\dotsb \hVar{x}_n]
  =(\hVar{x}_0\mBinOp(\hVar{x}_1\mBinOp\dotsb\mBinOp(\hVar{x}_n\mBinOp\mAVar)))
\end{equation}
これは\keyword{右畳み込み}と呼ばれる演算子である．\footnote{\haskell では $\mFoldRight^{*}_\mAVar\mList{x}$ を \code{foldr (*) xs a} と書く．引数の順序に注意しよう．}
...
\end{equation}
であると定義する．この演算子 $\mMapList$ をリストの\keyword{マップ演算子}と呼ぶ．\footnote{\haskell では $\hFunc{f}\mMapList\mList{x}$ を\code{map f xs} または \code{f <\$> xs} と書く．ただし演算子 \code{<\$>} は \code{fmap} 演算子の中置バージョンである．}
...
\begin{align}
  \hFunc{f}
  &=\mLambda\hVar{x}\mLambdaArrow\mConstant{100}+\hVar{x}\\
  \mList{x}
...
\begin{equation}
  \hFunc{f}\mAs{\hVar{x}:\mList{x}}{\mList{a}}%\overbrace{(\hVar{x}:\mList{x})}^{\mList{a}}
  =\dotsb
\end{equation}
として，変数 $\mList{a}$ でリスト全体を参照することも可能である．このような記法を\keyword{asパタン}と呼ぶ．\footnote{\haskell では $\hFunc{f}\mAs{\hVar{x}:\mList{x}}{\mList{a}}$ を \code{f as@(x:xs)} と書く．}
...
\item 左畳み込み演算子$\mFold^{\mBinOp}_a[\hVar{x}_0,\hVar{x}_1\dotsb \hVar{x}_n]$ は $a\mBinOp \hVar{x}_0\mBinOp \hVar{x}_1\mBinOp\dotsb\mBinOp \hVar{x}_n$ を返す．
\item 右畳み込み演算子 $\mFoldRight^{\mBinOp}_{a}[\hVar{x}_0,\hVar{x}_1\dotsb \hVar{x}_n]$ は$(\hVar{x}_0\mBinOp(\hVar{x}_1\mBinOp\dotsb\mBinOp(\hVar{x}_n\mBinOp a)))$ を返す．
...
\begin{equation}
  ...
  \begin{cases}
    ...
    &\hIfSo\hConstant{1}\\
    \_
    &\hIfSo\mFFact(\mAVar*\hVar{x})(\hVar{x}-\hConstant{1})
  \end{cases}
\end{equation}
...
関数 $\mFact$ と違い関数 $\mFFact$ は引数を2個とる．関数 $\mFFact$ を使って $\hVar{x}$ の階乗を求める場合は $\mFFact\,\hConstant{1}\,x$ と第1引数に $\hConstant{1}$ を与えることにする．この第1引数 $\mAVar$ はアキュムレータという．アキュムレータが演算の途中経過を引き渡していくイメージを描けば，末尾再帰の意味が理解できる
だろう．
...
\begin{align}
  ...
  &=\mAVar+\hAnonParam
\end{align}
というラムダ式があるとする．当然我々は関数 $\hFunc{f}$ がいつも $\hFunc{f}=\mConstant{100}+\hAnonParam$ であることを期待するし，\haskell においてはいつも保証される．\footnote{\haskell では
\begin{verbatim}
  a = 100
  f = \x -> a+x
\end{verbatim}
...
ところが，参照透過性のない言語，言い換えると変数への破壊的代入が許されている言語では，変数 $\mAVar$ の値がいつ変わっても不思議ではない．そこで，それらの言語では関数 $\hFunc{f}$ が定義された時点での $\mAVar$ の値を，関数 $\hFunc{f}$ の定義に含めておく．これがレキシカルクロージャの考え方である．
...
計算の途中で，計算にまつわる状態を残りの計算に引き継ぎたくなる場合がある．例えば，整数 $\hVar{x},\hVar{y},\mZVar$ があり $\hVar{x}=\hVar{y}/\mZVar$ なる値を続く計算で利用したいとする．だが $\mZVar\hIfEq\hConstant{0}$ のときには $\hVar{x}$ は正しく計算されない．こんなときプログラマが取れる手段は
\begin{itemize}
\item $\hVar{x}=\hVar{y}/\mZVar$ を計算した時点で\keyword{ゼロ除算例外}を発生させ，プログラムの制御を他の場所へ移す（大域ジャンプを行う）
...
整数 $\hVar{x},\hVar{y},\mZVar$ があり $\hVar{x}=\hVar{y}/\mZVar$ という式があるとする．この式は $\mZVar\hIfEq0$ のときにはゼロ除算エラーである．しかし「例外」は内部状態の書き換えであり，我々の計算に入れたくない．そこで変数 $\hVar{x}$ が正しく計算されたかもしれないし，されていないかもしれないということを $\mMaybe{u}$ のように?をつけた変数に入れて，忘れないようにしておこう．

ここで変数 $\mMaybe{u}$ が取り得る値は正しく計算された値 $\hVar{x}$ をラップしたものか，あるいはエラーを表す値 $\mNothing$ である．このように計算結果に「意味付け」をすることを\keyword{文脈}に入れると言う．定数 $\mNothing$ は「ナッシング」と呼ぶ．\footnote{\haskell では $\mNothing$ を \code{Nothing} と書く．}
...
% し $\hFunc{f}\mMap\mList{x}$ と書いてあれば $\hFunc{f}\mMapList\mList{x}$ のことであ
% るし，もし $\hFunc{f}\mMap\mMaybe{u}$ と書いてあれば $\hFunc{f}\mMapMaybe\mMaybe{u}$
...
% の部分に $\mMapList$ を，変数 $\hVar{x}$ がMaybeならば $\mMap$ の部分に
...
いま関数のリスト $[\hFunc{f},\hFunc{g},\mHFunc]$ と変数のリスト $[\hVar{x},\hVar{y},\mZVar]$ があるとする．リストのアプリカティブマップ演算子 $\mAppMapList$ を次のように定義する．
\begin{equation}
  [\hFunc{f},\hFunc{g},\mHFunc]\mAppMapList[\hVar{x},\hVar{y},\mZVar]
  =[\hFunc{f}\hVar{x},\hFunc{f}\hVar{y},\hFunc{f}\mZVar,\hFunc{g}\hVar{x},\hFunc{g}\hVar{y},\hFunc{g}\mZVar,\mHFunc\hVar{x},\mHFunc\hVar{y},\mHFunc\mZVar]
\end{equation}
...
\begin{equation}
  [\hFunc{f}]\mAppMapList{}[\hVar{x},\hVar{y},\mZVar]
  =[\hFunc{f}\hVar{x},\hFunc{f}\hVar{y},\hFunc{f}\mZVar]
\end{equation}
であり，通常のマップ演算子 $(\mMapList)$ を使ったマップすなわち
\begin{equation}
  \hFunc{f}\mMapList{}[\hVar{x},\hVar{y},\mZVar]
  =[\hFunc{f}\hVar{x},\hFunc{f}\hVar{y},\hFunc{f}\mZVar]
\end{equation}
...
% =\begin{cases}
% \mJustWith{\hFunc{f}\hVar{x}}
% &\mIf\left(\mMaybe{i}\hIfEq\mJustWith{f}\right)
% \mLogicalAnd
% \left(\mMaybe{u}\hIfEq\mJustWith{x}\right)\\
% \mNothing&\hOtherwise
% \end{cases}
% \end{equation}
...
いま関数 $\hFunc{f}\mIn\mProjEXP{\mR}{\mA }$ とは別な関数 $g\mIn\mProjEXP{\mA }{\mB }$ があったとしよう．関数 $\hFunc{f}$ と関数 $\hFunc{g}$ の合成 $\hFunc{g}\mCompFunc\hFunc{f}$ の型は
\begin{equation}
\hFunc{g}\mCompFunc\hFunc{f}\mIn\mProjEXP{\mR}{\mB }
\end{equation}
であるから，
\begin{equation}
...
\begin{equation}
  \hFunc{g}\mMapFunc\hFunc{f}
  =\mLambdaEXP{\hVar{x}}{\hFunc{g}(\hFunc{f}\hVar{x})}
\end{equation}
...
\begin{equation}
\hFunc{g}\mAppMapFunc\hFunc{f}=\mLambdaEXP{\hVar{x}}{\hFunc{g}\hVar{x}(\hFunc{f}\hVar{x})}
\end{equation}
...
&=\mLambdaEXP{\hVar{x}}{\mConstWith{g}\hVar{x}(\hFunc{f}\hVar{x})}\\
&=\mLambdaEXP{\hVar{x}}{(\mLambdaEXP{\_}{\hFunc{g}})\hVar{x}(\hFunc{f}\hVar{x})}\\
&=\mLambdaEXP{\hVar{x}}{\hFunc{g}(\hFunc{f}\hVar{x})}\\
&=\hFunc{g}\mCompFunc\hFunc{f}
\end{align}
であるからである．
...
% \left\{
% \begin{array}{ll}
% \mJustWith{\hFunc{f}\hVar{x}}&\mIf\mMaybe{u}\hIfEq\mJustWith{x}\\
% \mNothing&\hOtherwise
% \end{array}\right\}
% &\mIf\mMaybe{h}\hIfEq\mJustWith{f}\\
% \mNothing&\hOtherwise
% \end{cases}\\
% &=\begin{cases}
% \mJustWith{\hFunc{f}\hVar{x}}&\mIf\left(\mMaybe{u}\hIfEq\mJustWith{x}\right)\mLogicalAnd\left(\mMaybe{h}\hIfEq\mJustWith{f}\right)\\
% \mNothing&\hOtherwise
% \end{cases}
% \end{align}
% である．
...
\end{equation}
とする．この新しいマップ演算子 $\mBindMaybe$ のことをMaybeの\keyword{バインド演算子}と呼ぶ．ここで，もし計算が成功していたら $\mMaybe{v}=\mJustWith{\hFunc{g}\hVar{x}}$ であり，失敗していたら $\mMaybe{v}=\mNothing$ である．
...
\item 合成則: $(f\mCompFunc g)\mMap=(\hFunc{f}\mMap)\mCompFunc{}(g\mMap)$%=\hFunc{f}\mMap{}(g\mMap\hAnonParam)$
\end{enumerate}
...
\begin{align}
  ((\hFunc{f}\mMap)\mCompFunc(g\mMap))\mList{x}
...
% \item $\hFunc{f}\mMap=\mPureWith{f}\mAppMap$ --- （上述の四つの法則から導かれる）
\end{enumerate}
...
&=\mPureWith{\hFunc{g}\mCompFunc\mHFunc}\mAppMap v\\
&=\mPureWith{\hFunc{g}\mCompFunc\mHFunc\mZVar}
\end{align}
となる一方，合成則の右辺は
\begin{align}
...
であるから
\begin{equation}
k=i\mBind{}(j\hAnonParam)=\mPureWith{f\mCompFunc g\hAnonParam}
\end{equation}
を得る．ここで
\begin{align}
\mPureWith{f\mCompFunc g\hAnonParam}\mBind v
...
\begin{align}
\mConstWith{x}&=\mLambdaEXP{\_}{\hVar{x}}\\
\hFunc{g}\mAppMapFunc\hFunc{f}&=\mLambdaEXP{\hVar{x}}{\hFunc{g}\hVar{x}(\hFunc{f}\hVar{x})}
\end{align}
であった．
...
\begin{equation}
g\mBindFunc f=\mLambdaEXP{x}{g(\hFunc{f}\hVar{x})x}
\end{equation}
と定義する．ピュア演算子とバインド演算子から，関数のマップ演算子を
\begin{align}
\mConstWith{g\hAnonParam}\mBindFunc f
&=\mLambdaEXP{x}{\mConstWith{g\hAnonParam}(\hFunc{f}\hVar{x})x}\\
&=\mLambdaEXP{x}{\mConstWith{g(\hFunc{f}\hVar{x})}x}\\
&=\mLambdaEXP{x}{(\mLambdaEXP{\_}{g(\hFunc{f}\hVar{x})})x}\\
&=\mLambdaEXP{x}{g(\hFunc{f}\hVar{x})}\\
&=\hFunc{g}\mCompFunc f\\
&=\hFunc{g}\mMapFunc f
\end{align}
のように合成できる．
...
\hFunc{g}\mMapFunc\hFunc{f}&=\mLambdaEXP{x}{g(\hFunc{f}\hVar{x})}=\hFunc{g}\mCompFunc f\\
\hFunc{g}\mAppMapFunc\hFunc{f}&=\mLambdaEXP{x}{\hFunc{g}\hVar{x}(\hFunc{f}\hVar{x})}\\
g\mBindFunc f&=\mLambdaEXP{x}{g(\hFunc{f}\hVar{x})x}
\end{align}
である．
...
\begin{equation}
\mId\mCompFunc f=\hFunc{f}\mCompFunc\mId=\hFunc{f}
\end{equation}
...
\begin{equation}
  [\hFunc{f},\hFunc{g},\mHFunc]\mZip[x,\hVar{y},\mZVar]
...
\begin{equation}
  [\hFunc{f},\hFunc{g},\mHFunc]\mAppMapList[x,\hVar{y},\mZVar]
  =[\hFunc{f}\hVar{x},\hFunc{f}\hVar{y},\hFunc{f}\mZVar,\hFunc{g}\hVar{x},\hFunc{g}\hVar{y},\hFunc{g}\mZVar,\mHFunc\hVar{x},\mHFunc\hVar{y},\mHFunc\mZVar]
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    (\mZip)(\hVar{x}:\mList{x})(y:\mList{y})&=(x\mApply y):\mList{x}\mZip\mList{y}
  \end{aligned}
  \right.
\end{equation}
であると定義する．
...
\begin{equation}
  ...
  f\mCompCat g=\mJoinList\mCompFunc{}(\hFunc{f}\mMapList)\mCompFunc g
\end{equation}
とすれば，そのまま使えそうである．
...
\begin{multline}
  ...
  \begin{aligned}
    ...
    &=\mValueWith{NonDet}{\mJoinList\mCompFunc{}(\hFunc{f}\mMapList)\mCompFunc g}
  \end{aligned}
  \right.
\end{multline}
...
そこでリストのバインド演算子を用いると
\begin{align}
  \mJoinList\mCompFunc{}(\hFunc{f}\mMapList)\mCompFunc g
...
\begin{equation}
  y
  =\sharp\mCompFunc(\hFunc{f}\mMapMaybe)\mCompFunc g\mApply x
\end{equation}
...
\begin{equation}
  \mUncurry(\mBinOp)
  =\mLambda\mPairWith{\hVar{x}}{\hVar{y}}\mLambdaArrow x\mBinOp y
\end{equation}
...
\begin{equation}
  ...
  =
  \begin{bmatrix}
    f_{\hConstant{1},\hConstant{1}}\mCompFunc f_{2,1}\\
...
\begin{equation}
  ...
  =
  \begin{bmatrix}
    \hFunc{f}\mCompFunc g_1\\
    \hFunc{g}\mCompFunc g_2
  \end{bmatrix}
  \mWhere...
\end{equation}
...
\begin{equation}
  \mZVar=\hFunc{g}\mCompFunc \hFunc{f} x
\end{equation}

\begin{equation}
  \mZVar=\mathop{\mathrm{app}}\mPairWith{\mArrowWith{\mUncurry \hFunc{g}}\mCompCat\mFirstFunc\mArrowWith{\hFunc{f}}\mCompCat{}\mArrowWith{\left(\mLambda x\mLambdaArrow\mPairWith{x}{x}\right)}}{x}
\end{equation}
...
\begin{equation}
  ...
  =(\mJustWith{\hAnonParam}\mCompFunc(*2))\mBindComp(\mJustWith{\hAnonParam}\mCompFunc(*3))
\end{equation}

\begin{multline}
  \mLambda x\mLambdaArrow((\mLambda y\mLambdaArrow\mJustWith{y*x})\mBind{}\mJustWith{x*2})\\
  =(\mJustWith{\hAnonParam}\mCompFunc\mUncurry(*))\mBindComp(\mJustWith{\hAnonParam}\mCompFunc{}((*2),\mId))
\end{multline}
...
\begin{equation}
\mMainFunc=\hFunc{f}_n\mCompFunc f_{n-1}\mCompFunc\dotsb\mCompFunc \hFunc{f}
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    &(\mComplexTimes)a(x\mComplexPlus y)=\mAVar*\hVar{x}\mComplexPlus a*y
  \end{aligned}
  \right.
\end{equation}
...
\begin{equation}
f\circledcirc g=\mLambdaEXP{t}{\sum \hFunc{f}\hVar{x}*g(t-x)}
\end{equation}
...
$\hVar{x},\hVar{y},\mZVar$ で表すこととする．このことを数学者は $\hVar{x},\hVar{y},\mZVar\in\mRSet$ と書くが，
本書ではこれまで通り
\begin{equation}
x,\hVar{y},\mZVar\mIn\mRSet
\end{equation}
と表すことにする．

以下に実数の備える代数的性質を列挙する．どれも当たり前のことに見えるが，
ひとつひとつ見ていこう．ここで $\hVar{x},\hVar{y},\mZVar\mIn\mRSet$ とする．
\begin{description}
...
呼ぶ．
\item[実数の性質2. 加法の結合性] 任意の $\hVar{x},\hVar{y},\mZVar$ について
\begin{equation}
(\hVar{x}+\hVar{y})+\mZVar=x+(y+z)
\end{equation}
...
である．
\item[実数の性質7. 乗法の結合性] 任意の $\hVar{x},\hVar{y},\mZVar$ について
\begin{equation}
(x*y)*\mZVar=x*(y*z)
\end{equation}
である．
...
z\uparrow2=z*z
\end{equation}
であるから，いま $\mZVar=\hVar{x}+\hVar{y}$ とすると
\begin{align}
z\uparrow2&=z*z\\
&=z*(\hVar{x}+\hVar{y})\\
&=z*x+z*y\;\text{---分配律}\\
...
いま集合 $\mSet{G}$ があり，$\hVar{x},\hVar{y},\mZVar\mIn\mSet{G}$ であるとし，二項演算子
を $\mBinOp$ と書くことにして，体の性質の前半分を書き下してみよう．
\begin{description}
\item[性質1.] 任意の $\hVar{x}$ と任意の $\hVar{y}$ の演算の結果 $\hVar{x}\mBinOp y$ は
$\mSet{G}$ の元である．
\item[性質2.] 任意の $\hVar{x},\hVar{y},\mZVar$ について
\begin{equation}
...
特別な場合である．現代の数学では $\hVar{x}\mBinOp y\neq y\mBinOp x$ のように
...
単位的半群の性質は次の三つである．ただし $\hVar{x},\hVar{y},\mZVar$ が集合 $\mSet{M}$ の
元であるとする．
\begin{description}
\item[単位的半群の性質1.] 任意の $\hVar{x}$ と任意の $\hVar{y}$ の演算の結果
$\hVar{x}\mBinOp y$ は $\mSet{M}$ の元である．
\item[単位的半群の性質2. 結合性] 任意の $\hVar{x},\hVar{y},\mZVar$ について
\begin{equation}
(x\mBinOp y)\mBinOp \mZVar=x\mBinOp(y\mBinOp z)
\end{equation}
である．
...
$\hVar{x}\mBinOp y\mIn\mSet{M}$であった．見方を変えると，演算子$\mBinOp$とは
...
\begin{align}
t&=\mLambdaEXP{\hVar{x}\hVar{y}}{x}\\
f&=\mLambdaEXP{\hVar{x}\hVar{y}}{y}\\
\mIfFunc p\hVar{x}\hVar{y}&=p\hVar{x}\hVar{y}
\end{align}
本当かどうか試してみよう．
\begin{align}
\mIfFunc t\hVar{x}\hVar{y}&=t\hVar{x}\hVar{y}\\
&=(\mLambdaEXP{\hVar{x}\hVar{y}}{x})\hVar{x}\hVar{y}\\
&=x\\
\mIfFunc f\hVar{x}\hVar{y}&=\hFunc{f}\hVar{x}\hVar{y}\\
&=(\mLambdaEXP{\hVar{x}\hVar{y}}{y})\hVar{x}\hVar{y}\\
&=y
\end{align}
...
%double y_re = 3.0;
%double y_im = 4.0;
%double \mZVar_re = x_re + y_re;
%double \mZVar_im = x_im + y_im;
%printf(“%f, %fn”, \mZVar_re, \mZVar_im);
%\end{verbatim}
