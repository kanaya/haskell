...
%% Math-like Haskell (2024)

\include{haskell-styles}
...
\DeclareMathOperator{\mApply}{\mathop{\S}} % {\$}
...
リスト変数&&$\hListDecor{\hVar{x}}$\\ %%% !!!
Maybe変数&&$\hMaybeDecor{\hVar{x}}$\\ %%% !!!
Ether変数&&$\hEitherDecor{\hVar{x}}$\\ %%% !!!
一般のコンテナ変数&&$\hAnyContextDecor{\hVar{x}}$\\
...
有名なアクション&&$\hSpecialAction{main},\hSpecialAction{print}$\\ %%% !!!
\hline
型&ボールド（1文字）&$\hTypeName{a},\hTypeName{b},\hTypeName{c}$\\
有名な型&ボールド大文字&$\hBool,\hInt$\\
型クラス&ボールドイタリック大文字&$\hEq,\hOrd$\\
多相型&&$\hTypeConstruct{\hTypeConstructor{f}}{\hTypeName{a}}$\\
有名な多相型&&$\hMaybeConstruct{\hTypeName{a}}$\\
有名な多相型&特別な括弧で包む&$[\hTypeName{a}]$\\
集合&ブラックボード大文字&$\hSet{B},\hSet{Z},\hSet{R}$\\
\hline
型コンストラクタ&ボールドイタリック&$\hTypeConstructor{f},\hTypeConstructor{g},\hTypeConstructor{h}$\\
...
有名な値コンストラクタ&特別な括弧で包む&$\hListWith{\hVar{x}},\hPureWith{\hVar{x}},\hConstantWith{\hVar{x}}$\\
\hline
...
% アンボックス化タプル&丸括弧とシャープ記号で包む&$\mTupleUnboxedWith{x,\mYVar,\mZVar}$\\
\hline
\end{tabular}
\end{center}
\end{table*}
...
記号&意味&\haskell 記号\\
\hline\hline
$\hLambda, \hLambdaArrow$&ラムダ式&\verb|\|,\code{->}\\
$\hAnonParam$&無名パラメタ&\\
\hline
...
$\hNothing$&空Maybe（ナッシング）&\code{Nothing}\\
$\hPureNothing$&空&\\
\hline
\end{tabular}
\end{center}
...
\begin{equation}
  ...
  =(\mSin\mXVar)/\mXVar
\end{equation}
と書いても良い．この書き方を\keyword{ポイントフリースタイル}と呼ぶ．ポイントフリースタイルは今後頻出するので，是非覚えておいてもらいたい．\footnote{\haskell では $\mFFunc=\mLambda\mXVar\mLambdaArrow(\mSin\mXVar)/\mXVar$ を \code{f = \textbackslash x -> (sin x)/x} と書き，一方 $\mFFunc\mXVar=(\mSin\mXVar)/\mXVar$ を\code{f x = (sin x)/x} と書く．}
...
\end{align}
である．ただし $\mWhere$ 節は式ではないので，単独では使えないことに注意しよう．例えば $\mLet\mXXVar\mLetEq\mOneNumber+\mXVar\mLetIn\mFFunc\mXXVar$ は値を持つ式であるが，$\mWhere\mXXVar\mWhereEq\mOneNumber+\mXVar$ は値を持たない．
...
\item 関数定義にはパタンマッチが使える．例えば $f'0=1;f'x=(\sin x)/x$ と定義できる．\item パタンマッチは関数本体に書いても良い．関数 $f'$ の例で言えば $f'=\mCaseOf{\mXVar}0\mIfSo\mOneNumber;\_\mIfSo(\sin x)/x$ と 定義できる．\item 関数定義にはガードが使える．例えば $f''\mGuard{x<0}=0;f''\mGuard{x\equiv0}=1;f''\mGuard{\mOtherwise}=(\sin x)/x$ と定義できる．\item 条件分岐は $\mIfThenElseEXP{\mXVar}{\mYVar}{z}$ と書き，$\mXVar\equiv\mTrue$ の時には $\mYVar$ が，$\mXVar\equiv\mFalse$ のときには $\mZVar$ が式の値になる．\item 局所変数は $y=\mLetInEXP{x'}{\dotsb}{\mFFunc\mXVar'}$ または$y=\mFFunc\mXVar'\mWhereIsEXP{x'}{\dotsb}$ という書き方で導入できる．
\end{enumerate}
...
\end{equation}
と書ける．\footnote{\haskell では関数 \code{g} と関数 \code{f} の合成は \code{g.f} である．式 $\mZVar=\mGFunc\mCompFunc \mFFunc x$ は \code{z = g.f x} と書く．}
...
この場合変数 $\mXVar$ が $\mBoolType$ 型であることは自明であるため $\mXVar\mIn\mBoolType$ は省略できるが，可能な限り型を明記しておくことは良い習慣である．また必要に応じて変数に型名を注釈することがある．例えば $\mXVar=\mTrue\mIn\mBoolType$ のように書く場合がある．\footnote{\haskell では $\mXVar=\mTrue\mIn\mBoolType$ を \code{x = True :: Bool} と書く．}
...
ところで，整数全体の集合 $\mZSet$ には特別な元 $\mZeroNumber\mIn\mZSet$ がある．この元 $\mZeroNumber$ は $\mZVar\mIn\mZSet$ のとき
\begin{equation}
  \mZeroNumber+\mZVar
  =\mZVar+\mZeroNumber
  =\mZVar
\end{equation}
という性質を持つ．この $\mZeroNumber$ を演算 $+$ における\keyword{単位元}と呼ぶ．足し算のことを\keyword{加法}とも言うので $\mZeroNumber$ のことは\keyword{加法単位元}と呼ぶこともあるし，文字通り\keyword{零元}と呼ぶこともある．
...
組み合わせ $\mTupleWith{\mASet,\mPlus,\mZeroLeft,\mZeroRight}$ のことを\keyword{モノイド}または\keyword{単位的半群}と呼ぶ．例えば $\mTupleWith{\mZSet,+,\mZeroNumber,\mZeroNumber}$ はモノイドであるし，$\mTupleWith{\mZSet,*,\mOneNumber,\mOneNumber}$, $\mTupleWith{\mBSet,\mLogicalOr,\mFalse,\mFalse}$, $\mTupleWith{\mBSet,\mLogicalAnd,\mTrue,\mTrue}$ もモノイドである．これらのモノイドは全て左単位元と右単位元が同じなので，それぞれ $\mTupleWith{\mZSet,+,\mZeroNumber}$, $\mTupleWith{\mZSet,*,\mOneNumber}$, $\mTupleWith{\mBSet,\mLogicalOr,\mFalse}$, $\mTupleWith{\mBSet,\mLogicalAnd,\mTrue}$ とも書く．
...
\item 等差数列の配列は $[\mZeroNumber,\mOneNumber\dotsb10]$ のように途中を $\dotsb$ で省略できる．
\item 配列は無限長であってもよい．無限長配列は $[\mZeroNumber,\mOneNumber\dotsb]$ のように表記する．
...
\item 関数 $\mFFunc\mIn\mProjEXP{\mA }{\mA }$ はマップ演算子 $\mMapList$ を用いて $\mFFunc\mMapList[\mXVar_0,\mXVar_1\dotsb \mXVar_n]=[\mFFunc\mXVar_0,\mFFunc\mXVar_1\dotsb \mFFunc\mXVar_n]$ のようにリスト$[\mXVar_0,\mXVar_1\dotsb \mXVar_n]$ に適用できる．
...
関数は内部で自分自身を適用しても良い．例えば $\mXVar$ の階乗 $(\mXVar!)$ を返す関数 $\mFact$ は
\begin{equation}
  \mFact\mXVar
  =\mCaseOf{\mXVar}
  \begin{cases}
...
% し $\mFFunc\mMap\mList{x}$ と書いてあれば $\mFFunc\mMapList\mList{x}$ のことであ
% るし，もし $\mFFunc\mMap\mMaybe{u}$ と書いてあれば $\mFFunc\mMapMaybe\mMaybe{u}$
% のことであると自動的に解釈することである．そして，何の飾りもつけられ
% ていない変数 $\mXVar$ がふらっと現れ，目の前に $\mFFunc\mMap x$という式が登場し
...
いま関数 $\mFFunc\mIn\mProjEXP{\mR}{\mA }$ とは別な関数 $g\mIn\mProjEXP{\mA }{\mB }$ があったとしよう．関数 $\mFFunc$ と関数 $\mGFunc$ の合成 $\mGFunc\mCompFunc\mFFunc$ の型は
\begin{equation}
\mGFunc\mCompFunc\mFFunc\mIn\mProjEXP{\mR}{\mB }
\end{equation}
であるから，
\begin{equation}
...
\begin{equation}
  \mGFunc\mMapFunc\mFFunc
  =\mGFunc\mCompFunc\mFFunc
\end{equation}
としておこう．これは
\begin{equation}
  \mGFunc\mMapFunc\mFFunc
  =\mLambdaEXP{\mXVar}{\mGFunc(\mFFunc\mXVar)}
\end{equation}
...
\end{equation}
とする．これは，関数版のピュア演算子の定義と，一般マップ演算子と一般アプリカティブマップ演算子の関係 $\mFFunc\mMap\mVarContainer{w}=\mPureWith{f}\mAppMap\mVarContainer{w}$ から導かれる．すなわち
\begin{align}
\mConstWith{g}\mAppMapFunc\mFFunc
&=\mLambdaEXP{\mXVar}{\mConstWith{g}\mXVar(\mFFunc\mXVar)}\\
&=\mLambdaEXP{\mXVar}{(\mLambdaEXP{\_}{\mGFunc})\mXVar(\mFFunc\mXVar)}\\
&=\mLambdaEXP{\mXVar}{\mGFunc(\mFFunc\mXVar)}\\
&=\mGFunc\mCompFunc\mFFunc
\end{align}
であるからである．
...
\begin{equation}
\mVarContainer{v}=\mFFunc\mMap\mVarContainer{u}=\mPureWith{f}\mAppMap\mVarContainer{u}
\end{equation}
であった．
...
\item 合成則: $(f\mCompFunc g)\mMap=(\mFFunc\mMap)\mCompFunc{}(g\mMap)$%=\mFFunc\mMap{}(g\mMap\mAnonParam)$
\end{enumerate}
...
\begin{align}
  ((\mFFunc\mMap)\mCompFunc(g\mMap))\mList{x}
  &=(\mFFunc\mMap{}(g\mMap\mAnonParam))\mList{x}\\
  &=\mFFunc\mMap{}(g\mMap\mList{x})\\
  &=\mFFunc\mMap{}[\mGFunc\mXVar]\\
  &=[(f\mCompFunc g)x]\\
...
% \item $\mFFunc\mMap=\mPureWith{f}\mAppMap$ --- （上述の四つの法則から導かれる）
\end{enumerate}
...
\begin{equation}
  ...
  \mWhere\left\{
  \begin{aligned}
    \mFFunc\mMap{}\mJustWith{x}&=\mJustWith{\mFFunc\mXVar}\\
    \mFFunc\mMap\mNothing&=\mNothing
  \end{aligned}
  \right.
\end{equation}
...
\begin{equation}
  ...
  \left\{
  \begin{aligned}
    \mFFunc\mMap{}\mRightWith{x}&=\mRightWith{\mFFunc\mXVar}\\
    \mFFunc\mMap{}\mLeftWith{x}&=\mLeftWith{x}
  \end{aligned}
  \right.
\end{equation}
となる．\footnote{\haskell では
...
\begin{equation}
  [\mFFunc,\mGFunc,\mHFunc]\mZip[x,\mYVar,\mZVar]
  =[\mFFunc\mXVar,gy,hz]
\end{equation}
...
\begin{equation}
  ...
  f\mCompCat g=\mJoinList\mCompFunc{}(\mFFunc\mMapList)\mCompFunc g
\end{equation}
とすれば，そのまま使えそうである．
...
&\qquad\mList{f}\mLetEq[\mFFunc\mXVar\mListComp x\mFrom[\mZeroNumber,\mOneNumber\dotsb n]]\\
&\qquad\mList{g}\mLetEq[g(t-x)\mListComp x\mFrom[\mZeroNumber,\mOneNumber\dotsb n]]
\end{aligned}
\end{equation}
