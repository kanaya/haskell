...
整数型 $(\hInt)$ のリストは $[\hInt]$ と書き，整数のリスト型と呼ぶ．一般に $\hTypeName{a}$ 型のリストを $[\mA]$ と書く．仮の型である $\hTypeName{a}$ の事を \keyword{型パラメタ}と呼ぶ．

型 $\hTypeName{a}$ から型 $[\mA]$ を生成する演算子を\keyword{リスト型コンストラクタ}と呼んで$\mListTypeConstructor$ と書き
\begin{equation}
  [\mA]
...
$[\hVar{x}]$ のように $\hTypeName{a}$ 型の変数 $\hVar{x}$ を入れた$[\mA]$ 型の変数を作る演算子を\keyword{リスト値コンストラクタ}と呼ぶ．$[\mA]$ 型の変数のことを\keyword{リスト変数}とも呼ぶ．$\hTypeName{a}$ 型の変数 $\hVar{x}$ からリスト値コンストラクタを使ってリスト$\mList{x}$ を作ることは
\begin{equation}
  \mList{x}
  =[\hVar{x}]
\end{equation}
...
\end{equation}
とすることでアクセスできる．\footnote{\haskell では $\mList{x}\mListAt\hVar{n}$ を \code{xs!!n} と書く．}
...
\end{equation}
これは\keyword{右畳み込み}と呼ばれる演算子である．\footnote{\haskell では $\mFoldRight^{*}_{\hVar{a}}\mList{x}$ を \code{foldr (*) xs a} と書く．引数の順序に注意しよう．}
...
\begin{equation}
  ...
  \mMean\mList{x}
  =
  \left(\mLambda\hPairWith{\hVar{x}}{\hVar{y}}\mLambdaArrow x/y\right)\hPairWith{\mSum\mList{x}}{\mLength\mList{x}}
\end{equation}
...
\begin{equation}
  \hPairWith{\mSum\mList{x}}{\mLength\mList{x}}
  =\begin{bmatrix}
  \mSum\\
  \mLength
  \end{bmatrix}
  % \mathop{\square}{}
  \hPairWith{\mList{x}}{\mList{x}}
\end{equation}
という形にしたい．そうすれば
\begin{equation}
  \mSplit x
