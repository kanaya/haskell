...
記号&意味&\haskell 記号\\
\hline\hline
$\hxLambdaSyntax{\hxVar{x}}{\hxVar{x}}$&ラムダ式&\verb|\x -> x|\\
$\hxAnonParam$&無名パラメタ&\\
...
関数型プログラミングの特徴を一言で言えば，プログラム中の\keyword{破壊的代入}を禁止することである．変数 $\hxVar{x}$ に $\hxConstant{1}$ という数値が一度代入されたら，変数 $\hxVar{x}$ の値をプログラム中に書き換える，すなわち破壊的代入をすることはできない．この結果，変数の値はプログラムのどこでも，どの時点で読み出しても同じであることが保証される．これを変数の\keyword{参照透過性}と呼ぶ．
...
例えば引数 $\hxVar{x}$ をとり値 $\hxConstant{1}+\hxVar{x}$ を返すラムダ式を\python では
\begin{pythoncode}
\begin{verbatim}
lambda x: 1+x
\end{verbatim}
...
と書くが，我々はより簡潔に
\begin{equation}
  \hxLambdaSyntax{\hxVar{x}}{\hxConstant{1}+\hxVar{x}}
\end{equation}
と書くことにする．この式は多くの書物で $\lambda x\mathbin{.}1+x$ と記述されるところである．しかし我々はすべてのギリシア文字を変数名のために予約しておきたいのと，ピリオド記号 $(.)$ が今後登場する二項演算子 $\cdot$ と紛らわしいため，上述の記法を用いる．\footnote{\haskell ではラムダ式 $\hxLambdaSyntax{\hxVar{x}}{\hxConstant{1}+\hxVar{x}}$ を \code{\textbackslash x -> 1+x} と書く．ラムダ式は元々は $\Hat{x}\mathbin{.}x+1$ のように書かれていた．これが次第に $\mathop{\texttt{\textasciicircum}}x\mathbin{.}x+1$ となり，$\Lambda x\mathbin{.}x+1$ そして $\lambda x\mathbin{.}x+1$ に変化していったと言われている．\haskell が $\lambda$ の代わりに $\hxLambda$ 記号を使うのは，その形が似ているからである．}

ラムダ式は関数である．ラムダ式を適用するには，ラムダ式を括弧で包む必要がある．例を挙げる．
\begin{equation}
  (\hxLambdaSyntax{\hxVar{x}}{\hxConstant{1}+\hxVar{x}})\,\hxConstant{2}
\end{equation}
この式は結果として $\hxConstant{3}$ を返す．
...
本書では新たに，次のラムダ式記法も導入する．式中に記号 $\hxAnonParam$ が現れた場合，その式全体がラムダ式であるとみなす．記号 $\hxAnonParam$ の部分には引数が入る．第 $n$ 番目の $\hxAnonParam$ には第 $n$ 番目の引数が入る．例えばラムダ式 $\hxLambdaSyntax{\hxVar{x}\hxVar{y}}{\hxVar{x}+\hxVar{y}}$ は
\begin{equation}
  (\hxAnonParam+\hxAnonParam)
\end{equation}
と書いても良い．式を左から読んで1番目の $\hxAnonParam$ が元々の $\hxVar{x}$ すなわち第1引数を，2番目の $\hxAnonParam$ が元々の $\hxVar{y}$ すなわち第2引数を意味する．この省略記法はプログラミング言語\scheme における \code{cut} プロシジャに由来する．\footnote{\haskell では，中置演算子に限ってこの表現が使える．例えば$(\hxAnonParam+\hxAnonParam)$は単に \code{(+)} と表現できる．ただし\scheme における \code{cut} プロシジャの \code{<>} は\haskell にはないため，\scheme でいう \code{(cut f <> y)} に相当するコードを直接は書けない．\haskell ではその代わりに \code{(`f` y)} と書ける．}

\section{パタンマッチ・ガード・条件分岐}

関数の定義は，基本的にはラムダ式の変数への代入である．引数 $\hxVar{x}$ をとり値 $\hxConstant{2}*\hxVar{x}$ を返す関数 $\hxFunc{f}$ は
\begin{equation}
  \hxFunc{f}=\hxLambdaSyntax{\hxVar{x}}{\hxConstant{2}*\hxVar{x}}
\end{equation}
と定義できる．ただし，この省略形として
\begin{equation}
  \hxFunc{f}\hxVar{x}
  =\hxConstant{2}*\hxVar{x}
\end{equation}
と書いても良い．\footnote{\haskell では $\hxFunc{f}=\hxLambdaSyntax{\hxVar{x}}{\hxConstant{2}*\hxVar{x}}$ を \code{f = \textbackslash x -> 2*x} と書き，一方 $\hxFunc{f}\hxVar{x}=\hxConstant{2}*\hxVar{x}$ を\code{f x = 2*x} と書く．}

\separator

関数に\keyword{スペシャルバージョン}がある場合はそれらを列挙する．例えば引数が $\hxConstant{0}$ の場合は特別に戻り値が $\hxConstant{1}$ であり，その他の場合は関数 $\hxFunc{f}$ と同じ振る舞いをする関数 $\hxFunc{g}$ を考える．このとき $\hxFunc{g}$ は
\begin{equation}
  \left\{
  \begin{aligned}
    \hxFunc{g}\hxConstant{0}&=\hxConstant{1}\\
    \hxFunc{g}\hxVar{x}&=\hxConstant{2}*\hxVar{x}
  \end{aligned}
  \right.
\end{equation}
...
\begin{equation}
  ...
  \begin{cases}
    \hxConstant{0}&\hIfSo\hxConstant{1}\\
    \_&\hIfSo(\hxSin\hxVar{x})/\hxVar{x}
  \end{cases}
\end{equation}
のように書く．ここに $\_$ は任意の値の意味である．パタンマッチは上から順番にマッチングしていくため，この場合は $\hxConstant{0}$ 以外を意味する．
\footnote{\haskell では
\begin{verbatim}
  g x = case x of 0 -> 1
                  _ -> (sin x)/x
...
関数定義にパタンマッチではなく\keyword{場合分け}が必要な場合は\keyword{ガード}を用いる．例えば引数の値が負の場合は $\hxConstant{0}$ を，$\hxConstant{0}$ の場合は $\hxConstant{1}$ を，それ以外の場合は関数 $\hxFunc{f}$ と同じ振る舞いをする関数 $\hxFunc{h}$ は
\begin{equation}
  \begin{aligned}
    \hxFunc{h}\hxVar{x}&\hGuard{\hxVar{x}<\hxConstant{0}}=\hxConstant{0}\\
    &\hGuard{\hxVar{x}\hIfEq\hxConstant{0}}=\hxConstant{1}\\
    &\hGuard{\hOtherwise}=(\hxSin\hxVar{x})/\hxVar{x}
  \end{aligned}
\end{equation}
...
\begin{equation}
  \hxFunc{f}\hxVar{x}
  =\hIfSyntax{\hxVar{x}\hIfEq\hxConstant{0}}{\hxConstant{1}}{{}(\hxSin\hxVar{x})/\hxVar{x}}
\end{equation}
...
るため，結果として条件式の\keyword{短絡評価}が行われる点が異なる．\footnote{\haskell では $\hxFunc{f}\hxVar{x}=\hIfSyntax{\hxVar{x}\hIfEq\hxConstant{0}}{\hxConstant{1}}{{}(\hxSin\hxVar{x})/\hxVar{x}}$ を \code{f x = if x==0 then 1 else (sin x)/x} と書く．}

$\hIf$ 節の中身は真理値を返す関数であれば良いので，関数 $\hxFunc{p}$ を
\begin{equation}
  \hxFunc{p}
  =(\hxAnonParam\hIfEq\hxConstant{0})
\end{equation}
としておき，
\begin{equation}
  \hxFunc{f}\hxVar{x}
  =\hIfSyntax{\hxFunc{p}\hxVar{x}}{\hxConstant{1}}{{}(\hxSin\hxVar{x})/\hxVar{x}}
\end{equation}
と書く方法もしばしば用いられる．
...
\begin{equation}
  \hxVar{y}
  =\hxFunc{f}(\hxConstant{1}+\hxVar{x})
\end{equation}
という式のうち，先に $\hxConstant{1}+\hxVar{x}$ の部分を計算して $\hxVar{x'}$ のように名前をつけておきたいこともあるであろう．そんなときは
\begin{equation}
  \label{eq:letin}
  \hxVar{y}
  =\hLetSyntax{\hxVar{x'}\hLetEq\hxConstant{1}+\hxVar{x}}{\hxFunc{f}\hxVar{x'}}
\end{equation}
と書くことにする．このようにして導入された $\hxVar{x'}$ を\keyword{局所変数}と呼ぶ．\footnote{\haskell では $\hxVar{y}=\hLetSyntax{\hxVar{x'}\hLetEq\hxConstant{1}+\hxVar{x}}{\hxFunc{f}\hxVar{x'}}$ を \code{y = let x' = 1+x in f x'} と書く．}

式\eqref{eq:letin}は局所変数を後ろに回して
\begin{equation}
  \label{eq:where}
  \hxVar{y}
  =\hWhereSyntax{\hxFunc{f}\hxVar{x'}}{\hxVar{x'}\hLetEq\hxConstant{1}+\hxVar{x}}
\end{equation}
のように書いても良い．\footnote{\haskell では $\hxVar{y}=\hWhereSyntax{\hxFunc{f}\hxVar{x'}}{\hxVar{x'}\hLetEq\hxConstant{1}+\hxVar{x}}$ は \code{y = f x' where x' = 1+x} と書く．}
...
\begin{align}
  ...
  &=(\hxLambdaSyntax{\hxVar{x'}}{\hxFunc{f}\hxVar{x'}})(\hxFunc{g}\hxVar{x})
\end{align}
...
\item 関数はラムダ式で定義する．ラムダ式は $\hxLambdaSyntax{x}{(\sin x)/x}$ のように表記する．\item ラムダ式は $f\hxAnonParam$ のように無名パラメタ $\hxAnonParam$ を用いて表記しても良い．\item 関数定義 $f=\hxLambdaSyntax{x}{(\sin x)/x}$ は $\hxFunc{f}\hxVar{x}=(\sin x)/x$ と省略表記できる．
\item 関数定義にはパタンマッチが使える．例えば $f'0=1;f'x=(\sin x)/x$ と定義できる．\item パタンマッチは関数本体に書いても良い．関数 $f'$ の例で言えば $f'=\hCaseSyntax{\hxVar{x}}0\hIfSo\hxConstant{1};\_\hIfSo(\sin x)/x$ と 定義できる．\item 関数定義にはガードが使える．例えば $f''\hGuard{x<0}=0;f''\hGuard{x\hIfEq0}=1;f''\hGuard{\hOtherwise}=(\sin x)/x$ と定義できる．\item 条件分岐は $\mIfThenElseEXP{\hxVar{x}}{\hxVar{y}}{z}$ と書き，$\hxVar{x}\hIfEq\hTrue$ の時には $\hxVar{y}$ が，$\hxVar{x}\hIfEq\hFalse$ のときには $\hxVar{z}$ が式の値になる．\item 局所変数は $y=\mLetInEXP{x'}{\dotsb}{\hxFunc{f}\hxVar{x}'}$ または$y=\hxFunc{f}\hxVar{x}'\mWhereIsEXP{x'}{\dotsb}$ という書き方で導入できる．
\end{enumerate}
...
\begin{equation}
  ...
  =((\hAnyBinOp)\hxVar{x})\hxVar{y}
\end{equation}
であるから，部分適用が可能である．式\eqref{eq:star-x-y}から第2引数 $\hxVar{y}$ を取り除いて $(\hAnyBinOp)\hxVar{x}$ という「餓えた」1引数関数を取り出せる．例えば関数 $((+)\hxConstant{1})$ は引数に $\hxConstant{1}$ を加える関数である．\footnote{\haskell では $((+)1)$ を \code{((+)1)} と書く．}

\separator

前置される二項演算子 $(\hAnyBinOp)$ は，ラムダ式 $(\hxAnonParam\hAnyBinOp\hxAnonParam)$ の無名パラメタ $\hxAnonParam$ を省略したものと考えても良い．また $(\hxAnonParam\hAnyBinOp\hxVar{x})$ や $(\hxVar{x}\hAnyBinOp\hxAnonParam)$ から無名パラメタを省略した $(\hAnyBinOp\hxVar{x})$ と $(\hxVar{x}\hAnyBinOp)$ も有効な表現であり，特別に\keyword{セクション}と呼ばれる．

二項演算子 $\hAnyBinOp$ に対して $(\hAnyBinOp\hxVar{x})$ および $(\hxVar{x}\hAnyBinOp)$ はそれぞれ
\begin{align}
  (\hAnyBinOp\hxVar{x})
  &=(\hxAnonParam\hAnyBinOp\hxVar{x})\\
  (\hxVar{x}\hAnyBinOp)
  &=(\hxVar{x}\hAnyBinOp\hxAnonParam)=(\hAnyBinOp)\hxVar{x}
\end{align}
である．例えば $(\hxConstant{1}+)$ は $((+)\hxConstant{1})$ と等価であり，これは $(+\hxConstant{1})$ とも等価である．ただし，マイナス演算子$(-)$ だけは例外で，$(-\hxConstant{1})$ はマイナス $\hxConstant{1}$ を表す．負の数をいつも括弧で包んでおくのは良いアイディアである．\footnote{\haskell では $(1+)$ を\code{(1+)} と書く．また \code{(-1)} はセクションではなくマイナス$1$を表す（\code{-1} というリテラルとみなされる）．ただし \code{(- 1)}のように空白を挟んでも同じくマイナス$1$とみなされる（\code{1} というリテラルに単項マイナス演算子が適用される）．}
...
\begin{align}
  (\hxFunc{h}\hCompose\hxFunc{g})\hCompose\hxFunc{f}
    &=(\hxFunc{h}\hCompose\hxFunc{g})(\hxFunc{f}\hxAnonParam)\\
    &=\hxFunc{h}(\hxFunc{g}(\hxFunc{f}\hxAnonParam))\\
    &=\hxFunc{h}\hCompose(\hxFunc{g}\hCompose\hxFunc{f})
\end{align}
であるため，
\begin{equation}
...
\end{equation}
となる．演算子 $\hApply$ の優先順位は足し算よりも低いので $\hxFunc{f}(\hxVar{x}+\hxConstant{1})$ は $\hxFunc{f}\hApply\hxVar{x}+\hxConstant{1}$ と書くこともできる．演算子 $\hApply$ を閉じ括弧のいらない開き括弧と考えてもよい．\footnote{\haskell では $\hxFunc{g}\hApply\hxFunc{f}\hxVar{x}$ を \code{g \$ f x} と書く．}
...
\begin{align}
  \hxFunc{f}
  &=\hxFunc{g}\,\hxConstant{100}\\
  \hxVar{x}
  &=\hxFunc{f}\,\hxConstant{1}
\end{align}
とすれば $\hxVar{x}=\hxConstant{101}$ を得る．\footnote{\haskell では $ga=a+\hxAnonParam$ を $ga=\hxLambdaSyntax{x}{a+x}$ と展開しておいて \code{g a = \textbackslash x -> a+x} と書く．}
...
\begin{align}
  \hxVar{x}\uparrow\uparrow\hxVar{n}
  &\hGuard{\hxVar{n}\le\hxConstant{0}}=\hxConstant{1}\\
  &\hGuard{\hOtherwise}
  =\hxVar{x}\uparrow(\hxVar{x}\uparrow\uparrow(\hxVar{n}-\hxConstant{1}))
\end{align}
...
\item 任意の二項演算子 $\hAnyBinOp$ について $(\hAnyBinOp x)=\hxAnonParam\hAnyBinOp x$ であり，$(x\hAnyBinOp)=x\hAnyBinOp\hxAnonParam$ である．これらの記法をセクション記法と呼ぶ．
