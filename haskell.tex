...
% % $\hTypeName{a}thMaybeAppend$&Maybe結合\\
% \hline
$\hUnit$&ユニット&\code{()}\\
...
% $\hTypeName{a}ppMapList$&リストのアプリカティブマップ&\code{<*>}\\
% $\hTypeName{a}ppMapMaybe$&Maybeのアプリカティブマップ&\code{<*>}\\
% $\hTypeName{a}ppMapFunc$&関数のアプリカティブマップ&\code{<*>}\\
$\hMonadicBind$&モナドのバインド&\code{=<<}\\
% $\hTypeName{b}indList$&リストのバインド演算子&\code{=<<}\\
% $\hTypeName{b}indMaybe$&Maybeのバインド演算子&\code{=<<}\\
% $\hTypeName{b}indFunc$&関数のバインド演算子&\code{=<<}\\
\hline
$\hApply$&関数の適用&\code{\$}\\
$\hCompose$&関数の合成&\code{.}\\
...
\end{equation}
と書くことにする．この式は多くの書物で $\lambda x\hTypeName{a}thbin{.}1+x$ と記述されるところである．しかし我々はすべてのギリシア文字を変数名のために予約しておきたいのと，ピリオド記号 $(.)$ が今後登場する二項演算子 $\cdot$ と紛らわしいため，上述の記法を用いる．\footnote{\haskell ではラムダ式 $\hLambdaSyntax{\hVar{x}}{\hConstant{1}+\hVar{x}}$ を \code{\textbackslash x -> 1+x} と書く．ラムダ式は元々は $\Hat{x}\hTypeName{a}thbin{.}x+1$ のように書かれていた．これが次第に $\hTypeName{a}thop{\texttt{\textasciicircum}}x\hTypeName{a}thbin{.}x+1$ となり，$\Lambda x\hTypeName{a}thbin{.}x+1$ そして $\lambda x\hTypeName{a}thbin{.}x+1$ に変化していったと言われている．\haskell が $\lambda$ の代わりに $\hLambda$ 記号を使うのは，その形が似ているからである．}
...
% そこで，任意の型 $\hTypeName{a}$ について，組み合わせ $\hSingleTuppleWith{\hTypeName{a} ,\mPlus,\mZero}$ がモノイドである場合には
% \begin{equation}
% \hSingleTuppleWith{\hTypeName{a} ,\mPlus,\mZero}\hIsTypeOf\mMonoidTypeClass
% \end{equation}
...
Maybeとよく似た型にEitherがある．Maybeが $\hTypeName{a}$ 型または $\hNothing$ のいずれかの値をとったように，Eitherは $\hTypeName{a}$ 型または $\hTypeName{b}$ 型のいずれかの値を取る．$\hTypeName{a}$ 型または $\hTypeName{b}$ 型を取るEither型の変数 $\mEither{e}$ があるとすると，
\begin{equation}
  \mEither{e}
...
\begin{equation}
  \mEitherType{a}{b}
  =\mEitherTypeConstructor\,\hTypeName{a} \,\hTypeName{b}
\end{equation}
のように作られる．\footnote{\haskell では $\mEitherType{a}{b}$ も $\mEitherTypeConstructor\,\hTypeName{a}\,\hTypeName{b} $ も区別せずに\code{Either a b} と書く．}
...
Eitherはより複雑な計算エラーが発生する場合に用いる．Maybeが単に失敗を表す $\hNothing$ しか表現できなかったのに対し，Eitherは任意の型の変数で表現できる．習慣的に，正しい (right) 計算結果は $\mRightWith{x}$ 値コンストラクタで格納し，残された (left) エラーの情報は $\mLeftWith{x}$
値コンストラクタで格納する．
...
\begin{align}
\hPure\hId\hApplicativeMap\hAnyContextVar{x}&=\hAnyContextVar{x}\\
...
\begin{equation}
\hFunc{\psi}\hMonadicCompose\hFunc{\phi}=\hLambdaSyntax{\hAnyContextVar{x}}{\hFunc{\psi}\hMonadicBind(\hFunc{\phi}\hMonadicBind\hAnyContextVar{x})}
\end{equation}
...
\begin{equation}
\mDataTypePolymorphic\;\mEitherTypeConstructor\,\hTypeName{a} \,\hTypeName{b}
=\mLeftWith{\hTypeName{a} }
\mValueOr
\mRightWith{\hTypeName{b} }
\end{equation}
...
\begin{equation}
  \mInstanceDeclPolymorphic\;
  \mFunctorTypeClass\hHasElementsOf{}(\mEitherTypeConstructor\,\hTypeName{a})
  \mWhere{}
  \left\{
  \begin{aligned}
...
\begin{align}
  ...
  \mMean&=(\mTotal\hTypeName{a}thbin{\bigvee}\hTypeName{a}rrowWith{\mConst1}\Rrightarrow\mTotal)\Rrightarrow\hTypeName{a}rrowWith{\hUncurry(/)}
\end{align}
