...
有名な値コンストラクタ&特別な括弧で包む&$\hListWith{\hVar{x}},\hReturnWith{\hVar{x}}$\\
\hline
...
$\hAnonParam$&無名パラメタ&\\
\hline
$(+)\,\hVar{x}\,\hVar{y}$&関数化&\code{(+) x y}\\
$\hVar{x}\hInfixWith{\hFunc{f}}\hVar{y}$&中置演算&\code{x `f` y}\\
\hline
...
%%% $\hAnyBinOp$&任意の二項演算子&\\
%%% \hline
$\hForall{\hTypeName{s}}$&全量&\code{forall s.}\\
\hline
% $+$&一般加法演算子&\\
% \hline
