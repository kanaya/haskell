...
\def\specialColor{blue}
\def\varSpecialColor{green}
...
% Constants
\newcommand{\mConstant}[1]{#1}
\newcommand{\mSpecialConstant}[1]{\color{\specialColor}\textrm{#1}\color{\defaultColor}}
...
% Special variables
\newcommand{\mSpecialVar}[1]{\color{\specialColor}\textrm{#1}\color{\defaultColor}}
...
% Functions
\newcommand{\mSpecialFunc}[1]{\textrm{#1}}
\newcommand{\mSpecialFuncSmall}[1]{\color{\varSpecialColor}\mathrm{#1}\color{\defaultColor}}
...
\DeclareMathOperator{\mCos}{\mSpecialFunc{\color{\varSpecialColor}cos\color{\defaultColor}}}
...
\DeclareMathOperator{\mId}{\mSpecialFunc{\color{\varSpecialColor}id\color{\defaultColor}}}
...
\DeclareMathOperator{\mSin}{\mSpecialFunc{\color{\varSpecialColor}sin\color{\defaultColor}}}
...
% Actions
\newcommand{\mAction}[1]{#1}
\newcommand{\mActionLong}[1]{\textsl{#1}}
...
% Type constructors
\newcommand{\mTypeConstructor}[1]{\textrm{#1}}

\DeclareMathOperator{\mEitherTypeConstructor}{\mTypeConstructor{\color{\varSpecialColor}Either\color{\defaultColor}}}
\DeclareMathOperator{\mFuncTypeConstructor}{((\rightarrow)\mType{\color{\varSpecialColor}r\color{\defaultColor}})}
\DeclareMathOperator{\mListTypeConstructor}{\mTypeConstructor{\color{\varSpecialColor}List\color{\defaultColor}}}
\DeclareMathOperator{\mMaybeTypeConstructor}{\mTypeConstructor{\color{\varSpecialColor}Maybe\color{\defaultColor}}}
\DeclareMathOperator{\mZipListTypeConstructor}{\mTypeConstructor{\color{\varSpecialColor}ZipList\color{\defaultColor}}}
...
単位元&ローマンに斜線&$\mZero$\\
\hline
変数&イタリック（1文字）&$u,v,w,x,y,z$\\
リスト変数&$\mathrm{s}$をつける&$\mList{x}$\\
Maybe変数&$?$をつける&$\mMaybe{x}$\\
Ether変数&$!$をつける&$\mEither{x}$\\
一般のコンテナ変数&$*$をつける&$\mContainer{x}$\\
有名な変数&ローマン&$\mFirstVar,\mRestVar$\\
\hline
関数&イタリック（1文字）&$f,g,h,i,j,k$\\
一般のコンテナ関数&$*$をつける&$\mContainer{f}$\\
...
型コンストラクタ&ボールドイタリック&$\mPolymorphicTypeParameter{f},\mPolymorphicTypeParameter{g},\mPolymorphicTypeParameter{h}$\\
...
関数 $i$ をコンテナに入った変数 $u=\mPureWith{x}$ に適用させるのが一般バインド演算子 $(\mBind)$ の役割である．計算結果をコンテナ変数 $a$ に格納するとすると，関数 $i$ のコンテナ変数 $\mContainer{u}$ への適用は
\begin{equation}
\label{eq:i-love-u}
\mContainer{a}=i\mBind\mContainer{u}
\end{equation}
と書ける．うまく行けば $\mContainer{a}=\mPureWith{fx}$ となるし，そうでなければ $\mContainer{a}=\mPureNothing$ となる．

さて $\mContainer{u}=\mPureWith{x},\mContainer{v}=\mPureWith{y}$ として，かつ $y=fx$ であるとき
\begin{equation}
\mContainer{v}=f\mMap\mContainer{u}=\mPureWith{f}\mAppMap\mContainer{u}
\end{equation}
であった．
...
\begin{equation}
\mContainer{a}=\mPureWith{f\mAnonParam}\mBind\mContainer{u}
\end{equation}
となる．いま $y=fx$ であったから $\mContainer{a}=\mContainer{v}$ であり，最終的に
\begin{equation}
\mContainer{v}
=f\mMap\mContainer{u}
=\mPureWith{f}\mAppMap\mContainer{u}
=\mPureWith{f\mAnonParam}\mBind\mContainer{u}
\end{equation}
