...
% Typenames for functors
\newcommand{\functortypename}[1]{\mathcal{#1}}
\newcommand{\functortype}{\functortypename{F}}

% Functors
\newcommand{\functor}[1]{\langle#1\rangle}
\newcommand{\listtype}[1]{[#1]}
\newcommand{\maybe}[1]{\{#1\}}

% Functor names
\newcommand{\functorname}[1]{\textbf{#1}}
\newcommand{\listfunctor}{\mathop{\functorname{List}}}
\newcommand{\maybefunctor}{\mathop{\functorname{Maybe}}}
...
リスト$$l=[l_0,l_1\dotsb l_n]$$がある時，その総和を$$\sum l\triangleq l_0+l_1+\dotsb+l_n$$となるシグマ記号である．
この表現を一般化してみよう．
リスト$[l_0,l_1\dotsb l_n]$が与えられた時，$$\foldfunc_\circ^a[l_0,l_1\dotsb l_n]\triangleq a\circ l_0\circ l_1\circ\dotsb\circ l_n$$であると定義する．
この新しい記号は畳み込み演算子と呼ばれる．
...
型$\typename{a}$をコンテナに入れた型を一般に$\functor{\typename{a}}$と書くことにすると，一般化されたマップ演算子$\mapfunc$は$$\mapfunc::(\typename{a}\rightarrow\typename{b})\rightarrow\functor{\typename{a}}\rightarrow\functor{\typename{b}}$$という型を持つ．
...
一般に$$\typename{a}\mapsto\functor{\typename{a}}$$という風に型$\typename{a}$をコンテナに入れて型$\functor{\typename{a}}$を作る射を関手と呼ぶ．

関手全体の集合を$\functortype$で表す．
これまでに見た関手は$$\listfunctor,\maybefunctor::\functortype$$である．
