...
\hline\hline
定数&ローマン&$\hxOtherwise$\\
単位元（数学）&ローマンに斜線&$\hxMathZero$\\
\hline
変数&イタリック（1文字）&$\hxVar{x}$\\
リスト変数&sをつける&$\hxListVar{x}$\\
Maybe変数&?をつける&$\hxMaybeVar{x}$\\
Ether変数&!をつける&$\hxEitherVar{x}$\\
一般のコンテナ変数&星印をつける&$\hxContainerVar{x}$\\
有名な変数&ローマン&$\hxSpecialVar{first}$\\
タプル&丸括弧で包む&$\hxTupple{\hxVar{x}}{\hxVar{y}}$\\
集合（数学）&ブレースで包む&$\hxSet{\hxVar{x},\hxVar{y}}$\\
\hline
定数値コンストラクタ&ローマン大文字&$\hxTrue$\\
値コンストラクタ&ローマン大文字&$\hxJustValue{x}$\\
有名な値コンストラクタ&特別な括弧で包む&$\hxListValue{x}$\\
\hline
関数&イタリック（1文字）&$\hxFunc{f}$\\
有名な関数&ローマン&$\hxId$\\
関手（数学）&フラクチュール&$\hxMathId$\\
\hline
アクション&ギリシア文字（1文字）&$\hxAction{\alpha}$\\
有名なアクション&スラント&$\hxMain$\\
\hline
型&ボールド（1文字）&$\hxType{a}$\\
有名な型&ボールド大文字&$\hxBool$\\
型コンストラクタ&ボールド&$\hxTypeConstructor{\hxType{m}}{\hxType{a}}$\\
有名な型コンストラクタ&特別な括弧で包む&$\hxListTypeConstructor{\hxType{a}}$\\
型クラス&スモールキャピタル&$\hxEq$\\
% 型コンストラクタ&ボールドイタリック&$\hTypeConstructor{m},\hTypeConstructor{n}$\\
% 有名な型コンストラクタ&ボールド大文字&$\hList,\hMaybe$\\
集合（数学）&ブラックボード大文字&$\hxMathSetClass{B}$\\
\hline
\end{tabular}
\end{center}
\end{table*}
...
\hline\hline
キーワード&サンセリフ&$\hxIf$\\
特別なキーワード&サンセリフ太字&$\hxNewtype$\\
do記法中のキーワード&タイプライタ&$\hxDoLet$\\
\hline
\end{tabular}
\end{center}
\end{table*}
...
記号&意味&\haskell 記号\\
\hline\hline
$\hxLambdaExp{\hxVar{x}}{\hxVar{y}}$&ラムダ式&\verb|\x -> y|\\
$\hxAnonParam$&無名パラメタ&\\
$(\hxAnonParam+\hxAnonParam)\,\hxVar{x}\,\hxVar{y}$&関数化&\code{(+) x y}\\
$\hxVar{x}\hxInfix{\hxFunc{f}}\hxVar{y}$&中置演算子化&\code{x `f` y}\\
$\hxAnonTypeParam$&無名型パラメタ&\\
\hline
$\hxIsTypeOf$&型集合の元&\code{::}\\
$\hxIsElementOf$&値集合の元&\code{<-}\\
$\hxRightArrow$&射&\code{->}\\
$\hxCons$&cons演算子&\code{:}\\
$\hxAppend$&結合演算子&\code{++}\\
\hline
$\hxConvertFrom{\hxIntegral}$&型変換演算子&\code{fromIntegral}\\ %%% !!!
$\hxConvertTo{\hxEnum}$&同上&\code{toEnum}\\
\hline
$\hxFoldLeft$&左畳み込み&\code{foldl}\\
$\hxFoldRight$&右畳み込み&\code{foldr}\\
\hline
$\hxUnit$&ユニット&\code{()}\\
$\hxEmptyList$&空リスト&\code{[]}\\
$\hxNothing$&ナッシング&\code{Nothing}\\
$\hxPureNothing$&空&\\
\hline
$\hxMap$&リストのマップ&\code{`map`}\\
$\hxMapM$&文脈付きリストのマップ&\code{`mapM`}\\
$\hxFmap$&ファンクタのマップ&\code{<\$>}, \code{`fmap`}\\
$\hxAp$&アプリカティブマップ&\code{<*>}, \code{`ap`}\\
$\hxMonadicBind$&モナドのバインド&\code{=<<}\\
\hline
% $\hRightMonadicBind$&右バインド演算子&\code{>>=}\\
% $\hRightMonadicBindIgnore$&右バインド演算子（戻値無視）&\code{>>}\\
% \hline
$\hxApply$&関数の適用&\code{\$}\\
$\hxCompose$&関数の合成&\code{.}\\
$\hxMonadicCompose$&モナドの合成&\code{<=<}\\
% \hline
% $\hArrowArrow$&&\code{>>>}\\
% $\hArrowAnd$&&\code{\&\&\&}\\
% $\hArrowOr$&&\code{|||}\\
...
記号&意味&\haskell 記号\\
\hline\hline
$\hxForall$&全称量化子&\code{forall}\\
$\hxForallDot$&全称量化子の区切り&\code{.}\\
\hline
\end{tabular}
\end{center}
\end{table*}
...
\haskell プログラマもまた，多くの異なる概念を同じ貧弱な文字セットで表現しなければならない．これは，初めて\haskell コードを読むときに大きな問題になり得る．例えば\haskell では \code{[a]} という表記をよく扱う．この \code{[a]} は \code{a} という変数1要素からなるリストのこともあるし， \code{a} 型という仮の型から作ったリスト型の場合もあるが，字面からでは判断できない．もし変数はイタリック体，型はボールド体と決まっていれば，それぞれ $\hxListValue{a}$ および $\hxListTypeConstructor{\hxType{a}}$ と区別出来たところである．
...
関数型プログラミングの特徴を一言で言えば，プログラム中の\keyword{破壊的代入}を禁止することである．変数 $\hxVar{x}$ に $\hConstant{1}$ という数値が一度代入されたら，変数 $\hxVar{x}$ の値をプログラム中に書き換える，すなわち破壊的代入をすることはできない．この結果，変数の値はプログラムのどこでも，どの時点で読み出しても同じであることが保証される．これを変数の\keyword{参照透過性}と呼ぶ．
...
まずは\keyword{関数}から見ていくことにしよう．\python や一般的な数学書では引数 $\hxVar{x}$ をとる関数 $\hxFunc{f}$ を
\begin{pythoncode}
\begin{verbatim}
f(x)
\end{verbatim}
...
関数 $\hxFunc{f}$ に引数 $\hxVar{x}$を「食わせる」ことを\keyword{関数適用}と呼ぶ．もし $\hxFunc{f}\hxVar{x}$ と書いてあったら，それは $\hxFunc{f}$ と $\hxVar{x}$ の積，つまり $\hxFunc{f}*\hxVar{x}$ ではなく，従来の $\hxFunc{f}(\hxVar{x})$ すなわち関数 $\hxFunc{f}$ に引数 $\hxVar{x}$ を与えているものと解釈する．高校生向けの数学書でも $\sin x$ のように三角関数に限ってはカリー風に書くことになっているので，まるで馴染みがないということもないだろう．なお，関数はいつも引数の左側に書くことにする．これを「関数 $\hxFunc{f}$ が変数 $\hxVar{x}$ の\keyword{左から作用する}」と言い，また関数 $\hxFunc{f}$ のことを\keyword{左作用素}とも呼ぶ．
...
\end{verbatim}
\end{pythoncode}
と書くが，これも括弧が冗長なので今後は $\hxFunc{g}\hxVar{x}\hxVar{y}$ と書く．この場合式 $\hxFunc{g}\hxVar{x}\hxVar{y}$ は左を優先して結合する．つまり
\begin{equation}
  \hxFunc{g}\hxVar{x}\hxVar{y}=(\hxFunc{g}\hxVar{x})\hxVar{y}
\end{equation}
である．これは引数 $\hxVar{y}$ に関数 $(\hxFunc{g}\hxVar{x})$ が左から作用していると解釈する．関数 $(\hxFunc{g}\hxVar{x})$ は引数 $\hxVar{x}$ に関数 $\hxFunc{g}$ を作用させて作った関数である．引数に「飢えた」関数 $(\hxFunc{g}\hxVar{x})$ を\keyword{部分適用}された関数と呼ぶ．
...
$\hxMax$ は
\begin{equation}
  \hxMax\hxVar{x}\hxVar{y}
\end{equation}
として使われるが，関数適用は左結合であるから
\begin{equation}
  (\hxMax\hxVar{x})\hxVar{y}
\end{equation}
としても同じである．そこで $(\hxMax\hxVar{x})$ だけ取り出すと，これは「引数が $\hxVar{x}$ よりも小さければ $\hxVar{x}$ を，そうでなければ引数を返す関数」とみなすことができる．\footnote{\haskell では $\hxMax\hxVar{x}\hxVar{y}$ を \code{max x y} と書く．}

\section{ラムダ式}

関数の正体は\keyword{ラムダ式}である．ラムダ式とは，仮の引数をとり，その値をもとになにがしかの演算を行い，その結果を返す式である．ラムダ式は名前のない関数のようなものだ．それゆえ，無名関数と呼ばれることもある．

例えば引数 $\hxVar{x}$ をとり値 $\hConstant{1}+\hxVar{x}$ を返すラムダ式を\python では
\begin{pythoncode}
\begin{verbatim}
lambda x: 1+x
\end{verbatim}
...
と書くが，我々はより簡潔に
\begin{equation}
  \hLambdaSyntax{\hxVar{x}}{\hConstant{1}+\hxVar{x}}
\end{equation}
と書くことにする．この式は多くの書物で $\lambda x\mathbin{.}1+x$ と記述されるところである．しかし我々はすべてのギリシア文字を変数名のために予約しておきたいのと，ピリオド記号 $(.)$ が今後登場する二項演算子 $\cdot$ と紛らわしいため，上述の記法を用いる．\footnote{\haskell ではラムダ式 $\hLambdaSyntax{\hxVar{x}}{\hConstant{1}+\hxVar{x}}$ を \code{\textbackslash x -> 1+x} と書く．ラムダ式は元々は $\Hat{x}\mathbin{.}x+1$ のように書かれていた．これが次第に $\mathop{\texttt{\textasciicircum}}x\mathbin{.}x+1$ となり，$\Lambda x\mathbin{.}x+1$ そして $\lambda x\mathbin{.}x+1$ に変化していったと言われている．\haskell が $\lambda$ の代わりに $\hLambda$ 記号を使うのは，その形が似ているからである．}

ラムダ式は関数である．ラムダ式を適用するには，ラムダ式を括弧で包む必要がある．例を挙げる．
\begin{equation}
  (\hLambdaSyntax{\hxVar{x}}{\hConstant{1}+\hxVar{x}})\,\hConstant{2}
\end{equation}
この式は結果として $\hConstant{3}$ を返す．

複数引数をとるラムダ式は例えば
\begin{equation}
  \hLambdaSyntax{\hxVar{x}\hxVar{y}}{\hxVar{x}+\hxVar{y}}
\end{equation}
のように引数を並べて書く．

\separator

本書では新たに，次のラムダ式記法も導入する．式中に記号 $\hAnonParam$ が現れた場合，その式全体がラムダ式であるとみなす．記号 $\hAnonParam$ の部分には引数が入る．第 $n$ 番目の $\hAnonParam$ には第 $n$ 番目の引数が入る．例えばラムダ式 $\hLambdaSyntax{\hxVar{x}\hxVar{y}}{\hxVar{x}+\hxVar{y}}$ は
\begin{equation}
  (\hAnonParam+\hAnonParam)
\end{equation}
と書いても良い．式を左から読んで1番目の $\hAnonParam$ が元々の $\hxVar{x}$ すなわち第1引数を，2番目の $\hAnonParam$ が元々の $\hxVar{y}$ すなわち第2引数を意味する．この省略記法はプログラミング言語\scheme における \code{cut} プロシジャに由来する．\footnote{\haskell では，中置演算子に限ってこの表現が使える．例えば$(\hAnonParam+\hAnonParam)$は単に \code{(+)} と表現できる．ただし\scheme における \code{cut} プロシジャの \code{<>} は\haskell にはないため，\scheme でいう \code{(cut f <> y)} に相当するコードを直接は書けない．\haskell ではその代わりに \code{(`f` y)} と書ける．}

\section{パタンマッチ・ガード・条件分岐}

関数の定義は，基本的にはラムダ式の変数への代入である．引数 $\hxVar{x}$ をとり値 $(\hSin\hxVar{x})/\hxVar{x}$ を返す関数 $\hxFunc{f}$ は
\begin{equation}
  \hxFunc{f}=\hLambdaSyntax{\hxVar{x}}{(\hSin\hxVar{x})/\hxVar{x}}
\end{equation}
と定義できる．ただし，この省略形として
\begin{equation}
  \hxFunc{f}\hxVar{x}
  =(\hSin\hxVar{x})/\hxVar{x}
\end{equation}
と書いても良い．\footnote{\haskell では $\hxFunc{f}=\hLambdaSyntax{\hxVar{x}}{\hSin\hxVar{x})/\hxVar{x}}$ を \code{f = \textbackslash x -> (sin x)/x} と書き，一方 $\hxFunc{f}\hxVar{x}=(\hSin\hxVar{x})/\hxVar{x}$ を\code{f x = (sin x)/x} と書く．}

\separator

関数に\keyword{スペシャルバージョン}がある場合はそれらを列挙する．例えば引数が $\hConstant{0}$ の場合は特別に戻り値が $\hConstant{1}$ であり，その他の場合は関数 $\hxFunc{f}$ と同じ振る舞いをする関数 $\hxFunc{g}$ を考える．このとき $\hxFunc{g}$ は
\begin{equation}
  \left\{
  \begin{aligned}
    \hxFunc{g}\hConstant{0}&=\hConstant{1}\\
    \hxFunc{g}\hxVar{x}&=(\hSin\hxVar{x})/\hxVar{x}
  \end{aligned}
  \right.
\end{equation}
...
\begin{equation}
  \hxFunc{g}\hxVar{x}=\hCaseSyntax{\hxVar{x}}
  \begin{cases}
    \hConstant{0}&\hIfSo\hConstant{1}\\
    \_&\hIfSo(\hSin\hxVar{x})/\hxVar{x}
  \end{cases}
\end{equation}
...
\begin{verbatim}
  g x = case x of 0 -> 1
                  _ -> (sin x)/x
\end{verbatim}
または \code{g x = case x of \{0 -> 1; \_ -> (sin x)/x\}} と書く．}

一部のプログラミング言語では\keyword{デフォルト引数}という，引数を省略できるメカニズムがあるが，我々は引数をいつも省略しないことにする．\footnote{\haskell にもデフォルト引数はない．}

\separator

関数定義にパタンマッチではなく\keyword{場合分け}が必要な場合は\keyword{ガード}を用いる．例えば引数の値が負の場合は $\hConstant{0}$ を，$\hConstant{0}$ の場合は $\hConstant{1}$ を，それ以外の場合は関数 $\hxFunc{f}$ と同じ振る舞いをする関数 $\hxFunc{h}$ は
\begin{equation}
  \begin{aligned}
    \hxFunc{h}\hxVar{x}&\hGuard{\hxVar{x}<\hConstant{0}}=\hConstant{0}\\
    &\hGuard{\hxVar{x}\hIfEq\hConstant{0}}=\hConstant{1}\\
    &\hGuard{\hOtherwise}=(\hSin\hxVar{x})/\hxVar{x}
  \end{aligned}
\end{equation}
...
\begin{equation}
  \hxFunc{f}\hxVar{x}
  =\hIfSyntax{\hxVar{x}\hIfEq\hConstant{0}}{\hConstant{1}}{{}(\hSin\hxVar{x})/\hxVar{x}}
\end{equation}
...
るため，結果として条件式の\keyword{短絡評価}が行われる点が異なる．\footnote{\haskell では $\hxFunc{f}\hxVar{x}=\hIfSyntax{\hxVar{x}\hIfEq\hConstant{0}}{\hConstant{1}}{{}(\hSin\hxVar{x})/\hxVar{x}}$ を \code{f x = if x==0 then 1 else (sin x)/x} と書く．}

$\hIf$ 節の中身は真理値を返す関数であれば良いので，関数 $\hxFunc{p}$ を
\begin{equation}
  \hxFunc{p}
...
\begin{equation}
  \hxFunc{f}\hxVar{x}
  =\hIfSyntax{\hxFunc{p}\hxVar{x}}{\hConstant{1}}{{}(\hSin\hxVar{x})/\hxVar{x}}
\end{equation}
と書く方法もしばしば用いられる．

% 条件式は紙面が許せば次のように書いても良い．
% \begin{equation}
%   \hxFunc{f}\hxVar{x}=\begin{cases}
%   1&\mIf x\hIfEq0\\
%   (\sin x)/x&\hOtherwise
%   \end{cases}
% \end{equation}
...
\begin{equation}
  \hxVar{y}
  =\hxFunc{f}(\hConstant{1}+\hxVar{x})
\end{equation}
という式のうち，先に $\hConstant{1}+\hxVar{x}$ の部分を計算して $\hxVar{x'}$ のように名前をつけておきたいこともあるであろう．そんなときは
\begin{equation}
  \label{eq:letin}
  \hxVar{y}
  =\hLetSyntax{\hxVar{x'}\hLetEq\hConstant{1}+\hxVar{x}}{\hxFunc{f}\hxVar{x'}}
\end{equation}
と書くことにする．このようにして導入された $\hxVar{x'}$ を\keyword{局所変数}と呼ぶ．\footnote{\haskell では $\hxVar{y}=\hLetSyntax{\hxVar{x'}\hLetEq\hConstant{1}+\hxVar{x}}{\hxFunc{f}\hxVar{x'}}$ を \code{y = let x' = 1+x in f x'} と書く．}

式\eqref{eq:letin}は局所変数を後ろに回して
\begin{equation}
  \label{eq:where}
  \hxVar{y}
  =\hWhereSyntax{\hxFunc{f}\hxVar{x'}}{\hxVar{x'}\hLetEq\hConstant{1}+\hxVar{x}}
\end{equation}
のように書いても良い．\footnote{\haskell では $\hxVar{y}=\hWhereSyntax{\hxFunc{f}\hxVar{x'}}{\hxVar{x'}\hLetEq\hConstant{1}+\hxVar{x}}$ は \code{y = f x' where x' = 1+x} と書く．}

式\eqref{eq:letin}や式\eqref{eq:where}はラムダ式を使った\keyword{糖衣構文}（シンタックスシュガー）であり，一般に
\begin{align}
  \hLetSyntax{\hxVar{x'}\hLetEq\hxFunc{g}\hxVar{x}}{\hxFunc{f}\hxVar{x'}}
  &=\hWhereSyntax{\hxFunc{f}\hxVar{x'}}{\hxVar{x'}\hLetEq\hxFunc{g}\hxVar{x}}\\
  &=(\hLambdaSyntax{\hxVar{x'}}{\hxFunc{f}\hxVar{x'}})(\hxFunc{g}\hxVar{x})
\end{align}
である．ただし $\hWhere$ 節は式ではないので，単独では使えないことに注意しよう．例えば $\hLetSyntax{\hxVar{x'}\hLetEq\hxFunc{g}\hxVar{x}}{\hxFunc{f}\hxVar{x'}}$ は値を持つ式であるが，$\hWhereSyntax{}{\hxVar{x'}\hLetEq\hxFunc{g}\hxVar{x}}$ は値を持たない．

\section{この章のまとめ}

\begin{enumerate}
\item 変数 $\hxVar{x}$ に関数 $\hxFunc{f}$ を適用することを $\hxFunc{f}\hxVar{x}$ と書く．\item 変数 $\hxVar{x},\hxVar{y}$ に関数 $\hxFunc{g}$ を適用することを $g\hxVar{x}\hxVar{y}$ と書く．関数適用は左結合するので $g\hxVar{x}\hxVar{y}=(\hxFunc{g}\hxVar{x})y$ である．この関数 $(\hxFunc{g}\hxVar{x})$ は部分適用された関数と呼ぶ．
\item 関数はラムダ式で定義する．ラムダ式は $\hLambdaSyntax{x}{(\sin x)/x}$ のように表記する．\item ラムダ式は $f\hAnonParam$ のように無名パラメタ $\hAnonParam$ を用いて表記しても良い．\item 関数定義 $f=\hLambdaSyntax{x}{(\sin x)/x}$ は $\hxFunc{f}\hxVar{x}=(\sin x)/x$ と省略表記できる．
\item 関数定義にはパタンマッチが使える．例えば $f'0=1;f'x=(\sin x)/x$ と定義できる．\item パタンマッチは関数本体に書いても良い．関数 $f'$ の例で言えば $f'=\hCaseSyntax{\hxVar{x}}0\hIfSo\hConstant{1};\_\hIfSo(\sin x)/x$ と 定義できる．\item 関数定義にはガードが使える．例えば $f''\hGuard{x<0}=0;f''\hGuard{x\hIfEq0}=1;f''\hGuard{\hOtherwise}=(\sin x)/x$ と定義できる．\item 条件分岐は $\mIfThenElseEXP{\hxVar{x}}{\hxVar{y}}{z}$ と書き，$\hxVar{x}\hIfEq\hTrue$ の時には $\hxVar{y}$ が，$\hxVar{x}\hIfEq\hFalse$ のときには $\hxVar{z}$ が式の値になる．\item 局所変数は $y=\mLetInEXP{x'}{\dotsb}{\hxFunc{f}\hxVar{x}'}$ または$y=\hxFunc{f}\hxVar{x}'\mWhereIsEXP{x'}{\dotsb}$ という書き方で導入できる．
\end{enumerate}
...
\keyword{演算子}は関数の特別な姿である．演算子は\keyword{作用素}と呼んでも良い．どちらも英語のoperatorの和訳である．演算子は普通アルファベット以外のシンボル1個で表現し，変数や関数の前に置いて直後の変数や関数に作用させるか，2個の変数や関数の中間に置いてその両者に作用させる．例えば $-\hxVar{x}$ のマイナス記号 $(-)$ は変数の前に置いて直後の変数 $(\hxVar{x})$ に作用する演算子であり，$\hxVar{x}+\hxVar{y}$ のプラス記号 $(+)$ は2個の変数の間に置いてその両者 $(\hxVar{x},\hxVar{y})$ に作用する．
...
二項演算子は\keyword{中置}することが基本であるが，括弧で包むことで前置することも可能である．任意の二項演算子 $\hAnyBinOp$ について $\hxVar{x}\hAnyBinOp\hxVar{y}$ 及び $(\hAnyBinOp)\hxVar{x}\hxVar{y}$ は全く同じ意味である．すなわち
\begin{equation}
  \left(\hAnyBinOp\right)\hxVar{x}\hxVar{y}
  =\hxVar{x}\hAnyBinOp\hxVar{y}
\end{equation}
である．従って，二項演算子と2引数関数に本質的な差はない．本書では演算子と関数という用語は全く同じ意味で用いる．\footnote{\haskell では任意の二項演算子を括弧で包むことで前置演算子として使うことができる．例えば \code{\hxVar{x}+\hxVar{y}}と \code{(+)x y} は同じ結果を返す．逆に任意の2引数関数 \code{op}は \code{x `op` y} と書くことで中置することができる．}

一般の関数が左結合であることを思い出すと，二項演算子を関数に見立てた
$(\hAnyBinOp)$ も
\begin{equation}
  \label{eq:star-x-y}
  (\hAnyBinOp)\hxVar{x}\hxVar{y}
  =((\hAnyBinOp)\hxVar{x})\hxVar{y}
\end{equation}
であるから，部分適用が可能である．式\eqref{eq:star-x-y}から第2引数 $\hxVar{y}$ を取り除いて $(\hAnyBinOp)\hxVar{x}$ という「餓えた」1引数関数を取り出せる．例えば関数 $((+)\hConstant{1})$ は引数に $\hConstant{1}$ を加える関数である．\footnote{\haskell では $((+)1)$ を \code{((+)1)} と書く．}

\separator

前置される二項演算子 $(\hAnyBinOp)$ は，ラムダ式 $(\hAnonParam\hAnyBinOp\hAnonParam)$ の無名パラメタ $\hAnonParam$ を省略したものと考えても良い．また $(\hAnonParam\hAnyBinOp\hxVar{x})$ や $(\hxVar{x}\hAnyBinOp\hAnonParam)$ から無名パラメタを省略した $(\hAnyBinOp\hxVar{x})$ と $(\hxVar{x}\hAnyBinOp)$ も有効な表現であり，特別に\keyword{セクション}と呼ばれる．

二項演算子 $\hAnyBinOp$ に対して $(\hAnyBinOp\hxVar{x})$ および $(\hxVar{x}\hAnyBinOp)$ はそれぞれ
\begin{align}
  (\hAnyBinOp\hxVar{x})
  &=(\hAnonParam\hAnyBinOp\hxVar{x})\\
  (\hxVar{x}\hAnyBinOp)
  &=(\hxVar{x}\hAnyBinOp\hAnonParam)=(\hAnyBinOp)\hxVar{x}
\end{align}
...
一般の関数 $\hxFunc{f}$ を中置演算子に変換する記号 $\hInfixWith{\hxFunc{f}}$ を今後用いる．この記号を用いると値 $\hxFunc{f}\hxVar{x}\hxVar{y}$ のことを $\hxVar{x}\hInfixWith{\hxFunc{f}}\hxVar{y}$ と書くことができる．\footnote{\haskell では $\hxVar{x}\hInfixWith{\hxFunc{f}}\hxVar{y}$ を \code{x `f` y} と書く．}
...
\begin{equation}
  \label{eq:ffx}
  \hxVar{z}=
  \hxFunc{g}(\hxFunc{f}\hxVar{x})
\end{equation}
である．式\eqref{eq:ffx}から括弧を省略して $\hxVar{z}=\hxFunc{g}\hxFunc{f}\hxVar{x}$ としてしまうと，関数適用は左結合するから $\hxVar{z}=(\hxFunc{g}\hxFunc{f})\hxVar{x}$ の意味になってしまう．関数 $\hxFunc{g}$ が引数に関数を取るのでない限り $(\hxFunc{g}\hxFunc{f})$ は無意味なので，式\eqref{eq:ffx}の括弧は省略できない．

ここで，引数のことは忘れて，関数 $\hxFunc{f}$ と関数 $\hxFunc{g}$ を先に\keyword{合成}しておきたいとしよう．その合成を $\hxFunc{g}\hCompose\hxFunc{f}$ と書く．演算子 $\hCompose$ は\keyword{関数合成演算子}と呼ぶ．合成はラムダ式を使って
\begin{equation}
  \hxFunc{g}\hCompose \hxFunc{f}
  =\hxFunc{g}(\hxFunc{f}\hAnonParam)
\end{equation}
と定義できる．関数合成演算子 $\hCompose$ は関数適用よりも優先順位が高く， $\left(\hxFunc{g}\hCompose \hxFunc{f}\right)\hxVar{x}$ は単に $\hxFunc{g}\hCompose\hxFunc{f}\hxVar{x}$ と書いても良い．この記法は括弧の数を減らすためにしばしば用いられる．式\eqref{eq:ffx}は関数合成演算子を用いると
\begin{equation}
  \hxVar{z}
  =\hxFunc{g}\hCompose\hxFunc{f}\hxVar{x}
\end{equation}
と書ける．\footnote{\haskell では関数 \code{g} と関数 \code{f} の合成は \code{g.f} である．式 $\hxVar{z}=\hxFunc{g}\hCompose \hxFunc{f} x$ は \code{z = g.f x} と書く．}

\separator

関数合成演算子は，連続して用いることができる．関数合成演算子は左結合するので，関数 $\hxFunc{f},\hxFunc{g},\hxFunc{h}$ について
\begin{equation}
...
\begin{align}
  (\hxFunc{h}\hCompose\hxFunc{g})\hCompose\hxFunc{f}
    &=(\hxFunc{h}\hCompose\hxFunc{g})(\hxFunc{f}\hAnonParam)\\
    &=\hxFunc{h}(\hxFunc{g}(\hxFunc{f}\hAnonParam))\\
    &=\hxFunc{h}\hCompose(\hxFunc{g}\hCompose\hxFunc{f})
\end{align}
であるため，
\begin{equation}
...
\begin{equation}
  \hxFunc{f}\hApply\hxVar{x}
  =\hxFunc{f}\hxVar{x}
\end{equation}
演算子 $\hApply$ の優先順位は関数適用も含めあらゆる演算子よりも低いものとする．関数適用演算子を用いて式\eqref{eq:ffx}を書き直すと
\begin{equation}
  \hxVar{z}
  =\hxFunc{g}\hApply\hxFunc{f}\hxVar{x}
\end{equation}
となる．演算子 $\hApply$ の優先順位は足し算よりも低いので $\hxFunc{f}(\hxVar{x}+\hConstant{1})$ は $\hxFunc{f}\hApply\hxVar{x}+\hConstant{1}$ と書くこともできる．演算子 $\hApply$ を閉じ括弧のいらない開き括弧と考えてもよい．\footnote{\haskell では $\hxFunc{g}\hApply\hxFunc{f}\hxVar{x}$ を \code{g \$ f x} と書く．}

関数適用演算子のもう一つの興味深い使い方は，関数適用演算子の部分適用である．セクション $(\hApply\hxVar{x})$ を用いると
\begin{equation}
  (\hApply\hxVar{x})\hxFunc{f}
  =\hxFunc{f}\hApply\hxVar{x}
\end{equation}
であるから，関数適用演算子を用いて引数を関数に渡すことができる．\footnote{\haskell では $(\hApply\hxVar{x})\hxFunc{f}$ を \code{(\$x)f} と書く．}

\section{高階関数}

関数を引数に取ったり，あるいは関数を返す関数のことを\keyword{高階関数}と呼ぶことがある．関数合成演算子と関数適用演算子は高階関数の好例である．

他に例えば，引数として整数 $\hxVar{a}$ を取り，関数 $\hxFunc{f}\hxVar{x}=\hxVar{a}+\hxVar{x}$ を返すような関数 $\hxFunc{g}$ を
\begin{equation}
  \hxFunc{g}\hxVar{a}
  =\hxVar{a}+\hAnonParam
\end{equation}
...
\begin{align}
  \hxFunc{f}
  &=\hxFunc{g}\,\hConstant{100}\\
  \hxVar{x}
  &=\hxFunc{f}\,\hConstant{1}
\end{align}
とすれば $\hxVar{x}=\hConstant{101}$ を得る．\footnote{\haskell では $ga=a+\hAnonParam$ を $ga=\hLambdaSyntax{x}{a+x}$ と展開しておいて \code{g a = \textbackslash x -> a+x} と書く．}
...
計算機科学者ドナルド・クヌースは，整数 $\hxVar{x},\hxVar{n}$ が与えられたとき $\hxVar{x}$ の $\hxVar{n}$ 乗を ${\hxVar{x}}^{\hxVar{n}}$ ではなく $\hxVar{x}\uparrow\hxVar{n}$ と書いた．これは
\begin{equation}
  \hxVar{x}\uparrow\hxVar{n}
  =\underbrace{\hxVar{x}*\hxVar{x}*\dotsb*\hxVar{x}}_{\hxVar{n}}
\end{equation}
という意味である．\footnote{\haskell では $\hxVar{x}\uparrow\hxVar{n}$ を \code{x\textasciicircum n} と書く．}

クヌースはさらに演算子 $\uparrow\uparrow$ を
\begin{equation}
  \hxVar{x}\uparrow\uparrow\hxVar{n}
  =\underbrace{\hxVar{x}\uparrow\hxVar{x}\uparrow\dotsb\uparrow\hxVar{x}}_{\hxVar{n}}
\end{equation}
のように定義した．これを\keyword{クヌースの矢印}と呼ぶ．クヌースの矢印は
\begin{align}
  \hxVar{x}\uparrow\uparrow\hxVar{n}
  &\hGuard{\hxVar{n}\le\hConstant{0}}=\hConstant{1}\\
  &\hGuard{\hOtherwise}
  =\hxVar{x}\uparrow(\hxVar{x}\uparrow\uparrow(\hxVar{n}-\hConstant{1}))
\end{align}
...
\begin{enumerate}
\item 関数と演算子は同じものである．
\item 任意の二項演算子 $\hAnyBinOp$ について $\hxVar{x}\hAnyBinOp y$ と $(\hAnyBinOp)\hxVar{x}\hxVar{y}$ は全く同じ意味である．
...
\item 関数 $\hxFunc{f}$ と関数 $\hxFunc{g}$ を合成した関数を $f\hCompose g$ と書く．
\item 式 $f\hApply \hxFunc{g}\hxVar{x}$ は $f(\hxFunc{g}\hxVar{x})$ の意味である．
...
\begin{equation}
  \hLambdaSyntax{\hxVar{x}\hxVar{y}}{\hxVar{x}+\hxVar{y}}
  =\hLambdaSyntax{\hxVar{x}}{\left(\hLambdaSyntax{\hxVar{y}}{\hxVar{x}+\hxVar{y}}\right)}
\end{equation}
...
ところで，整数全体の集合 $\hSet{Z}$ には特別な元 $\hConstant{0}\hIsTypeOf\hSet{Z}$ がある．この元 $\hConstant{0}$ は $\hxVar{z}\hIsTypeOf\hSet{Z}$ のとき
\begin{equation}
  \hConstant{0}+\hxVar{z}
  =\hxVar{z}+\hConstant{0}
  =\hxVar{z}
...
\begin{equation}
  \hId=\hAnonParam
\end{equation}
これはもちろん $\hId=\hLambdaSyntax{\hxVar{x}}{\hxVar{x}}$ の意味で，引数をそのまま返す関数である．
...
\begin{equation}
  \hListVar{x}
  =\hListWith{\hxVar{x}*\hConstant{2}\hListComprehension\hxVar{x}\hFrom\hListWith{\hConstant{0},\hConstant{1}\dotsb\hConstant{9}}}
\end{equation}
のように書こう．ここに右辺のリストから一つずつ要素を取り出して左辺に代入する演算子$\hFrom$を用いた．\footnote{\haskell では $  \hListVar{x}=\hListWith{\hxVar{x}*\hConstant{2}\hListComprehension\hxVar{x}\hFrom\hListWith{\hConstant{0},\hConstant{1}\dotsb\hConstant{9}}}
$ を \code{xs = [x*2 | x<-[0,1..9]]} と書く．}

内包表記の式は複数あっても良い．例えば
\begin{equation}
  \hListVar{x}
  =[\hxVar{x}+\hxVar{y}\mListComp\hxVar{x}\mFrom[\hConstant{0},\hConstant{1}\dotsb\hConstant{9}],\,\hxVar{y}\mFrom[\hConstant{0},\hConstant{1}\dotsb\hConstant{5}],\,\hxVar{x}+\hxVar{y}>\hConstant{3}]
\end{equation}
は $\hConstant{0}\le\hxVar{x}\le\hConstant{9}$ かつ $\hConstant{0}\le\hxVar{y}\le\hConstant{5}$ の範囲で $\hxVar{x}+\hxVar{y}>\hConstant{3}$となる $\hxVar{x}$ 及び $\hxVar{y}$ から $\hxVar{x}+\hxVar{y}$ を並べたリストである．これは\python でいう
\begin{pythoncode}
\begin{verbatim}
...
\begin{align}
  \hxFunc{f}
  &=\mLambda\hxVar{x}\mLambdaArrow\hConstant{100}+\hxVar{x}\\
  \hListVar{x}
...
\begin{equation}
  \hxVar{n}!=\hxVar{n}(\hxVar{n}-\hConstant{1})(\hxVar{n}-\hConstant{2})\dots\hConstant{1}
\end{equation}
...
\begin{equation}
  \left\{
    \begin{split}
      ...
      \hxVar{n}!&=\hxVar{n}(\hxVar{n}-\hConstant{1})!
    \end{split}
  \right.
\end{equation}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    &\hIfSo\hConstant{1}\\
    \_
    &\hIfSo\hxVar{x}*\hFact(\hxVar{x}-\hConstant{1})
  \end{cases}
\end{equation}
...
と書いてみよう．この定義の末尾の式は
\begin{equation}
  (*)\hxVar{x}(\hFact(\hxVar{x}-\hConstant{1}))
\end{equation}
...
\begin{equation}
  ...
  \begin{cases}
    ...
    &\hIfSo\hxVar{a}\\
    \_
    &\hIfSo\hFactPrime(\hxVar{a}*\hxVar{x})(\hxVar{x}-\hConstant{1})
  \end{cases}
\end{equation}
...
% 関数 $\hFact$ と違い関数 $\hFactPrime$ は引数を2個とる．関数 $\hFactPrime$ を使って $\hxVar{x}$ の階乗を求める場合は $\hFactPrime\,\hConstant{1}\,x$ と第1引数に $\hConstant{1}$ を与えることにする．この第1引数 $\hxVar{a}$ はアキュムレータという．アキュムレータが演算の途中経過を引き渡していくイメージを描けば，末尾再帰の意味が理解できるだろう．
...
\haskell は，意図しない限り遅延評価を行う．これは特に左畳み込み演算子 $(\hFold)$ を使う場合に問題となる．いま $\hListVar{x}=[\hxVar{x}_0,\hxVar{x}_1,\hxVar{x}_2,\hxVar{x}_3]$ とすると，左畳み込み演算 $\hFold^+_{\hConstant{0}}\hListVar{x}$ は
\begin{align}
  \hFold^+_{\hConstant{0}}\hListVar{x}
  &=\hFold^+_{\hConstant{0}}{}(\hxVar{x}_0:\hxVar{x}_1:\hxVar{x}_2:{\hEmptyList})\\
  &=\hFold^+_{\hConstant{0}+\hxVar{x}_0}{}(\hxVar{x}_1:\hxVar{x}_2:\hxVar{x}_3:{\hEmptyList})\\
  &=\hFold^+_{(\hConstant{0}+\hxVar{x}_0)+\hxVar{x}_1}{}(\hxVar{x}_2:\hxVar{x}_3:{\hEmptyList})\\
  &=\hFold^+_{((\hConstant{0}+\hxVar{x}_0)+\hxVar{x}_1)+\hxVar{x}_2}{}(\hxVar{x}_3:{\hEmptyList})\\
  &=\hFold^+_{(((\hConstant{0}+\hxVar{x}_0)+\hxVar{x}_1)+\hxVar{x}_2)+\hxVar{x}_3}{}{\hEmptyList}\\
  &=(((\hConstant{0}+\hxVar{x}_0)+\hxVar{x}_1)+\hxVar{x}_2)+\hxVar{x}_3
\end{align}
と展開される．遅延評価のために，\haskell 処理系は値ではなく式をメモリにストアしなければならないが，左畳み込み演算は大きなメモリを必要としがちである．もし例えば予め $\hConstant{0}+\hxVar{x}_0$ を先に計算しておくなど左畳み込みだけ先に評価しておけば，大いにメモリの節約になる．そのために\haskell は「遅延評価無し」の左畳み込み演算子を用意している．\footnote{「遅延評価無し」の左畳み込み演算子を\haskell では \code{foldl'} と書く．}
...
\begin{align}
  ...
  &=\hxVar{a}+\hAnonParam
\end{align}
というラムダ式があるとする．当然我々は関数 $\hxFunc{f}$ がいつも $\hxFunc{f}=\hConstant{100}+\hAnonParam$ であることを期待するし，\haskell においてはいつも保証される．\footnote{\haskell では
\begin{verbatim}
  a = 100
  f = \x -> a+x
\end{verbatim}
...
計算の途中で，計算にまつわる状態を残りの計算に引き継ぎたくなる場合がある．例えば，整数 $\hxVar{x},\hxVar{y},\hxVar{z}$ があり $\hxVar{z}=\hxVar{y}/\hxVar{x}$ なる値を続く計算で利用したいとする．だが $\hxVar{x}\hIfEq\hConstant{0}$ のときには $\hxVar{z}$ は正しく計算されない．こんなときプログラマが取れる手段は
\begin{itemize}
\item $\hxVar{z}=\hxVar{y}/\hxVar{x}$ を計算した時点で\keyword{ゼロ除算例外}を発生させ，プログラムの制御を他の場所へ移す（大域ジャンプを行う）
\item グローバル変数にゼロ除算エラーが起こったことを記録しておき，$\hxVar{z}$ にはとりあえずの数値，例えば $\hConstant{0}$ を代入しておいて，計算を続行させる
...
整数 $\hxVar{x}$ からMaybe値 $\hMaybeVar{u}=\hJustWith{\hxVar{x}}$ を作り，関数 $\hxFunc{g}\hxVar{x}=\hConstant{1}+\hxVar{x}$ をMaybe値 $\hMaybeVar{u}$ に食わせてMaybe値 $\hMaybeVar{v}$ ただし
\begin{equation}
  \hMaybeVar{v}
...
\begin{equation}
\hxFunc{\psi}\hMonadicCompose\hxFunc{\phi}=\hxFunc{\psi}{}\hMonadicBind(\hxFunc{\phi}\hMonadicBind\hAnonParam)
\end{equation}
...
\begin{align*}
\hAnyContextVar{z}
&=(\hLambdaSyntax{\hxVar{y'}}{(\hLambdaSyntax{\hxVar{x'}}{\hAction{\phi}\hxVar{x'}\hxVar{y'}})\hMonadicBind\hAnyContextVar{x}})\hMonadicBind\hAnyContextVar{y}\\
...
\begin{equation}
  \hxFunc{g}\mMapFunc\hxFunc{f}
  =\hLambdaSyntax{\hxVar{x}}{\hxFunc{g}(\hxFunc{f}\hxVar{x})}
\end{equation}
...
\begin{equation}
\hxFunc{g}\hTypeName{a}ppMapFunc\hxFunc{f}=\hLambdaSyntax{\hxVar{x}}{\hxFunc{g}\hxVar{x}(\hxFunc{f}\hxVar{x})}
\end{equation}
...
&=\hLambdaSyntax{\hxVar{x}}{\mConstWith{g}\hxVar{x}(\hxFunc{f}\hxVar{x})}\\
&=\hLambdaSyntax{\hxVar{x}}{(\hLambdaSyntax{\_}{\hxFunc{g}})\hxVar{x}(\hxFunc{f}\hxVar{x})}\\
&=\hLambdaSyntax{\hxVar{x}}{\hxFunc{g}(\hxFunc{f}\hxVar{x})}\\
&=\hxFunc{g}\hCompose\hxFunc{f}
\end{align}
であるからである．
...
式\eqref{eq:def-of-i}から $ix=\mPureWith{\hxFunc{f}\hxVar{x}}$ すなわち $i=\mPureWith{f\hAnonParam}$ の関係を抜き出すと式\eqref{eq:i-love-u}は
\begin{equation}
...
\begin{equation}
  ...
  =\hxFunc{f}\hTypeName{b}ind(g\hAnonParam)
\end{equation}
...
\item 合成則: $(f\hCompose g)\mMap=(\hxFunc{f}\mMap)\hCompose{}(g\mMap)$%=\hxFunc{f}\mMap{}(g\mMap\hAnonParam)$
\end{enumerate}
...
\begin{align}
  ...
  &=(\hxFunc{f}\mMap{}(g\mMap\hAnonParam))\hListVar{x}\\
...
\begin{align}
\mConstWith{x}&=\hLambdaSyntax{\_}{\hxVar{x}}\\
\hxFunc{g}\hTypeName{a}ppMapFunc\hxFunc{f}&=\hLambdaSyntax{\hxVar{x}}{\hxFunc{g}\hxVar{x}(\hxFunc{f}\hxVar{x})}
\end{align}
であった．
...
\begin{equation}
g\hTypeName{b}indFunc f=\hLambdaSyntax{x}{g(\hxFunc{f}\hxVar{x})x}
\end{equation}
と定義する．ピュア演算子とバインド演算子から，関数のマップ演算子を
\begin{align}
\mConstWith{g\hAnonParam}\hTypeName{b}indFunc f
&=\hLambdaSyntax{x}{\mConstWith{g\hAnonParam}(\hxFunc{f}\hxVar{x})x}\\
&=\hLambdaSyntax{x}{\mConstWith{g(\hxFunc{f}\hxVar{x})}x}\\
&=\hLambdaSyntax{x}{(\hLambdaSyntax{\_}{g(\hxFunc{f}\hxVar{x})})x}\\
&=\hLambdaSyntax{x}{g(\hxFunc{f}\hxVar{x})}\\
&=\hxFunc{g}\hCompose f\\
&=\hxFunc{g}\mMapFunc f
\end{align}
のように合成できる．
...
\hxFunc{g}\mMapFunc\hxFunc{f}&=\hLambdaSyntax{x}{g(\hxFunc{f}\hxVar{x})}=\hxFunc{g}\hCompose f\\
\hxFunc{g}\hTypeName{a}ppMapFunc\hxFunc{f}&=\hLambdaSyntax{x}{\hxFunc{g}\hxVar{x}(\hxFunc{f}\hxVar{x})}\\
g\hTypeName{b}indFunc f&=\hLambdaSyntax{x}{g(\hxFunc{f}\hxVar{x})x}
\end{align}
である．
...
関数はモノイドとしての性質も持つ．関数 $\mId=\hAnonParam$ とすると，任意の関数 $\hxFunc{f}$ に対して
\begin{equation}
...
\end{aligned}
\end{equation}
とする．ここで $\hConstant{0}\le x\le\pi$ の範囲で $\hxVar{y}$ の値を求めたいとしよう．$\hxVar{x}$
の範囲を $n+1$ 分割するとする．
...
&\qquad\hListVar{f}\mLetEq[\hxFunc{f}\hxVar{x}\mListComp x\mFrom[\hConstant{0},\hConstant{1}\dotsb n]]\\
...
\begin{equation}
f\circledcirc g=\hLambdaSyntax{t}{\sum \hxFunc{f}\hxVar{x}*g(t-x)}
\end{equation}
...
$$
y=\sum_x\hxFunc{f}\hxVar{x}*g(\hAnonParam-x)
$$
...
\begin{align}
t&=\hLambdaSyntax{\hxVar{x}\hxVar{y}}{x}\\
f&=\hLambdaSyntax{\hxVar{x}\hxVar{y}}{y}\\
\mIfFunc p\hxVar{x}\hxVar{y}&=p\hxVar{x}\hxVar{y}
\end{align}
本当かどうか試してみよう．
\begin{align}
\mIfFunc t\hxVar{x}\hxVar{y}&=t\hxVar{x}\hxVar{y}\\
&=(\hLambdaSyntax{\hxVar{x}\hxVar{y}}{x})\hxVar{x}\hxVar{y}\\
&=x\\
\mIfFunc f\hxVar{x}\hxVar{y}&=\hxFunc{f}\hxVar{x}\hxVar{y}\\
&=(\hLambdaSyntax{\hxVar{x}\hxVar{y}}{y})\hxVar{x}\hxVar{y}\\
&=y
\end{align}
