...
\newcommand{\foldfunc}{\bigoplus} % foldl
...
\mapfunc^{f\cdot g}&\equiv\mapfunc^f\cdot\mapfunc^g
\end{align}
...
\applicativefunc^fg&=\backslash x\rightarrow fx(gx)
\end{align*}
...
\listtype{x}\bind f&=\concat\mapfunc^fx
\end{align*}

Function:
\begin{align*}
\monadreturn x&=\backslash y\rightarrow x\\
g\bind f&=\backslash h\rightarrow f(gh)h
\end{align*}
...
(\monad{x}\bind f)\bind g&=\monad{x}\bind((\backslash x'\rightarrow fx')\bind g)
\end{align}
