...
\begin{equation}
  \mFunc{f}\mXVar
\end{equation}
と書くことにする．\footnote{\haskell では関数 \code{f} に引数 \code{x}を適用させることを \code{f x} と書く．数学や物理学では $x$ をパラメタとする関数を $f(x)$ と書く場合もあるし，$f$ のようにパラメタを省略する場合もある．パラメタを省略した場合は，$fx_0$ の意味で $f\vert_{x=x_0}$ と書くことがある．}
