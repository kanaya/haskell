...
有名な型コンストラクタ&ボールド大文字&$\mListTypeConstructor,\mMaybeTypeConstructor$\\
...
関数型プログラミングの特徴を一言で言えば，プログラム中の\keyword{破壊的代入}を禁止することである．変数 $\mXVar$ に $1$ という数値が一度代入されたら，変数 $\mXVar$ の値をプログラム中に書き換える，すなわち破壊的代入をすることはできない．この結果，変数の値はプログラムのどこでも，どの時点で読み出しても同じであることが保証される．これを変数の\keyword{参照透過性}と呼ぶ．
...
まずは\keyword{関数}から見ていくことにしよう．\python や一般的な数学書では引数 $\mXVar$ をとる関数 $\mFFunc$ を
\begin{pythoncode}
\begin{verbatim}
f(x)
\end{verbatim}
...
\begin{enumerate}
\item 変数 $\mXVar$ に関数 $\mFFunc$ を適用することを $\mFFunc\mXVar$ と書く．\item 変数 $\mXVar,\mYVar$ に関数 $\mGFunc$ を適用することを $g\mXVar\mYVar$ と書く．関数適用は左結合するので $g\mXVar\mYVar=(\mGFunc\mXVar)y$ である．この関数 $(\mGFunc\mXVar)$ は部分適用された関数と呼ぶ．
\item 関数はラムダ式で定義する．ラムダ式は $\mLambdaEXP{x}{(\sin x)/x}$ のように表記する．\item ラムダ式は $f\mAnonParam$ のように無名パラメタ $\mAnonParam$ を用いて表記しても良い．\item 関数定義 $f=\mLambdaEXP{x}{(\sin x)/x}$ は $\mFFunc\mXVar=(\sin x)/x$ と省略表記できる．
\item 関数定義にはパタンマッチが使える．例えば $f'0=1;f'x=(\sin x)/x$ と定義できる．\item パタンマッチは関数本体に書いても良い．関数 $f'$ の例で言えば $f'=\mCaseOf{x}0\mIfSo1;\_\mIfSo(\sin x)/x$ と 定義できる．\item 関数定義にはガードが使える．例えば $f''\mGuard{x<0}=0;f''\mGuard{x\equiv0}=1;f''\mGuard{\mOtherwise}=(\sin x)/x$ と定義できる．\item 条件分岐は $\mIfThenElseEXP{\mXVar}{\mYVar}{z}$ と書き，$\mXVar\equiv\mTrue$ の時には $\mYVar$ が，$\mXVar\equiv\mFalse$ のときには $\mZVar$ が式の値になる．\item 局所変数は $y=\mLetInEXP{x'}{\dotsb}{\mFFunc\mXVar'}$ または$y=\mFFunc\mXVar'\mWhereIsEXP{x'}{\dotsb}$ という書き方で導入できる．
\end{enumerate}
...
\begin{equation}
  ...
  =((\mBinOp)\mXVar)\mYVar
\end{equation}
であるから，部分適用が可能である．式\eqref{eq:star-x-y}から第2引数 $\mYVar$ を取り除いて $(\mBinOp)x$ という「餓えた」1引数関数を取り出せる．例えば関数 $((+)1)$ は引数に $1$ を加える関数である．\footnote{\haskell では $((+)1)$ を \code{((+)1)} と書く．}
...
\end{align}
である．例えば $(\mOneNumber+)$ は $((+)\mOneNumber)$ と等価であり，これは $(+\mOneNumber)$ とも等価である．ただし，マイナス演算子$(-)$ だけは例外で，$(-\mOneNumber)$ はマイナス $1$ を表す．負の数をいつも括弧で包んでおくのは良いアイディアである．\footnote{\haskell では $(1+)$ を\code{(1+)} と書く．また \code{(-1)} はセクションではなくマイナス$1$を表す（\code{-1} というリテラルとみなされる）．ただし \code{(- 1)}のように空白を挟んでも同じくマイナス $1$ とみなされる（\code{1} というリテラルに単項マイナス演算子が適用される）．}
...
二つの整数 $z_1,z_2\in\mZSet$ があるとしよう．両者の間には\keyword{足し算} $(+)$ が定義されており，その結果すなわち\keyword{和}もまた整数である．ここで
\begin{equation}
  z_1+z_2
  \in\mZSet
\end{equation}
...
一般に，$a,\mZeroLeft,\mZeroRight\in\mASet$ として
\begin{equation}
  \label{eq:identity}
  \mZeroLeft\mPlus a
  =a\mPlus\mZeroRight
...
\begin{equation}
  \mList{x}
  =[\mXVar*2\mListComp\mXVar\mFrom[0,1\dots9]]
\end{equation}
のように書こう．ここに右辺のリストから一つずつ要素を取り出して左辺に代入する演算子$\mFrom$を用いた．\footnote{\haskell では $\mList{x}=[\mXVar*2\mListComp\mXVar\mFrom[0,1\dots9]]$ を \code{xs = [x*2 | x<-[0,1..9]]} と書く．}

内包表記の式は複数あっても良い．例えば
\begin{equation}
  \mList{x}
  =[\mXVar+\mYVar\mListComp\mXVar\mFrom[0,1\dots9],\,\mYVar\mFrom[0,1\dots5],\,\mXVar+\mYVar>3]
\end{equation}
は $0\le\mXVar\le9$ かつ $0\le\mYVar\le5$ の範囲で $\mXVar+\mYVar>3$となる $\mXVar$ 及び $\mYVar$ から $\mXVar+\mYVar$ を並べたリストである．これは\python でいう
\begin{pythoncode}
\begin{verbatim}
...
\begin{equation}
  \mFoldRight^{\mBinOp}_\mAVar[\mXVar_0,\mXVar_1\dotsb \mXVar_n]
  =(\mXVar_0\mBinOp(\mXVar_1\mBinOp\dotsb\mBinOp(\mXVar_n\mBinOp\mAVar)))
\end{equation}
...
\begin{equation}
  \mFold_\mEmptyList^{\mAppend}[[\mZeroNumber,\mOneNumber,\mTwoNumber,[\mThreeNumber,\mFourNumber,\mFiveNumber]\dotsb]
  =[\mZeroNumber,\mOneNumber,\mTwoNumber,\mThreeNumber,\mFourNumber,\mFiveNumber\dotsb]
\end{equation}
...
\begin{align}
  \mFFunc
  &=\mLambda x\mLambdaArrow\mConstant{100}+\mXVar\\
  \mList{x}
...
\begin{equation}
  \mFirstVar:\mRestVar
\end{equation}
を用い，リファレンス $\mFirstVar$ はリストが保持する型，リファレンス $\mRestVar$ はリスト型であるとする．演算子 $:$ を\lisp に倣って\keyword{cons演算子}と呼ぶ．\footnote{\haskell でも要素 \code{x} をリスト \code{xs} の先頭に追加することを \code{\mXVar:xs} と書く．}
...
\begin{equation}
  \mFFunc\mAs{\mXVar:\mList{x}}{\mList{a}}%\overbrace{(\mXVar:\mList{x})}^{\mList{a}}
  =\dotsb
\end{equation}
...
\begin{enumerate}
\item 配列はリストである．配列は $[\mXVar_0,\mXVar_1,\mXVar_2]$ のように表記する．
...
\item リストは先頭要素と続くリストから定義される．先頭要素を $\mXVar$ とし，続くリストを $\mList{x}$ とすると$\mXVar:\mList{x}$ はリストである．ここに演算子 $:$ は結合(cons)演算子である．
\item リストはリスト結合(append)できる．リスト $\mList{y}$ とリスト $\mList{z}$ のリスト結合は $\mList{y}\mAppend\mList{z}$ である．
\item 左畳み込み演算子$\mFold^{\mBinOp}_a[\mXVar_0,\mXVar_1\dotsb \mXVar_n]$ は $a\mBinOp \mXVar_0\mBinOp \mXVar_1\mBinOp\dotsb\mBinOp \mXVar_n$ を返す．
\item 右畳み込み演算子 $\mFoldRight^{\mBinOp}_{a}[\mXVar_0,\mXVar_1\dotsb \mXVar_n]$ は$(\mXVar_0\mBinOp(\mXVar_1\mBinOp\dotsb\mBinOp(\mXVar_n\mBinOp a)))$ を返す．
\item 平坦化演算子 $\mJoinList=\mFold_\mEmptyList^{\mAppend}$ はリストを平坦化する．
\item 関数 $f\mIn\mProjEXP{\mA }{\mA }$ はマップ演算子 $\mMapList$ を用いて $f\mMapList[\mXVar_0,\mXVar_1\dotsb \mXVar_n]=[\mFFunc\mXVar_0,\mFFunc\mXVar_1\dotsb \mFFunc\mXVar_n]$ のようにリスト$[\mXVar_0,\mXVar_1\dotsb \mXVar_n]$ に適用できる．
\item マップ演算子$\mMapList$ の型は$\mProjEXP{(\mProjEXP{\mA }{\mB})}{\mProjEXP{\mA }{\mB }}$である．
\item $\mHead(\mXVar:\mList{x})=x,\mTail(\mXVar:\mList{x})=\mList{x}$である．
\end{enumerate}
...
関数は内部で自分自身を適用しても良い．例えば $\mXVar$ の階乗 $(x!)$ を返す関数 $\mFact$ は
\begin{equation}
  \mFact x
  =\mCaseOf{x}
  \begin{cases}
...
関数 $\mFact$ と違い関数 $\mFact'$ は引数を2個とる．関数 $\mFact'$ を使って $\mXVar$ の階乗を求める場合は $\mFact'1x$ と第1引数に $1$ を与えることにする．この第1引数 $a$ はアキュムレータという．アキュムレータが演算の途中経過を引き渡していくイメージを描けば，末尾再帰の意味が理解できる
だろう．
...
\haskell は，意図しない限り遅延評価を行う．これは特に左畳み込み演算子 $(\mFold)$ を使う場合に問題となる．いま $\mList{x}=[\mXVar_0,\mXVar_1,\mXVar_2,\mXVar_3]$ とすると，左畳み込み演算 $\mFold^+_0\mList{x}$ は
\begin{align}
  \mFold^+_0\mList{x}
  &=\mFold^+_0{}(\mXVar_0:\mXVar_1:\mXVar_2:\mEmptyList)\\
  &=\mFold^+_{0+\mXVar_0}{}(\mXVar_1:\mXVar_2:\mXVar_3:\mEmptyList)\\
  &=\mFold^+_{(0+\mXVar_0)+\mXVar_1}{}(\mXVar_2:\mXVar_3:\mEmptyList)\\
  &=\mFold^+_{((0+\mXVar_0)+\mXVar_1)+\mXVar_2}{}(\mXVar_3:\mEmptyList)\\
  &=\mFold^+_{(((0+\mXVar_0)+\mXVar_1)+\mXVar_2)+\mXVar_3}{}\mEmptyList\\
  &=(((0+\mXVar_0)+\mXVar_1)+\mXVar_2)+\mXVar_3
\end{align}
と展開される．遅延評価のために，\haskell 処理系は値ではなく式をメモリにストアしなければならないが，左畳み込み演算は大きなメモリを必要としがちである．もし例えば予め $0+\mXVar_0$ を先に計算しておくなど左畳み込みだけ先に評価しておけば，大いにメモリの節約になる．そのために\haskell は「遅延評価無し」の左畳み込み演算子を用意している．\footnote{「遅延評価無し」の左畳み込み演算子を\haskell では \code{oldl'} と書く．}
...
計算の途中で，計算にまつわる状態を残りの計算に引き継ぎたくなる場合がある．例えば，整数 $\mXVar,y,z$ があり $\mXVar=y/z$ なる値を続く計算で利用したいとする．だが $z\equiv0$ のときには $\mXVar$ は正しく計算されない．こんなときプログラマが取れる手段は
\begin{itemize}
\item $\mXVar=y/z$ を計算した時点で\keyword{ゼロ除算例外}を発生させ，プログラムの制御を他の場所へ移す（大域ジャンプを行う）
\item グローバル変数にゼロ除算エラーが起こったことを記録しておき，$\mXVar$ にはとりあえずの数値，例えば $0$ を代入しておいて，計算を続行させる
...
整数 $\mXVar,y,z$ があり $\mXVar=y/z$ という式があるとする．この式は $z\equiv0$ のときにはゼロ除算エラーである．しかし「例外」は内部状態の書き換えであり，我々の計算に入れたくない．そこで変数 $\mXVar$ が正しく計算されたかもしれないし，されていないかもしれないということを $\mMaybe{u}$ のように?をつけた変数に入れて，忘れないようにしておこう．
...
整数 $\mXVar$ からMaybe値 $\mMaybe{u}=\mJustWith{x}$ を作り，関数 $\mGFunc\mXVar=1+x$ をMaybe値 $\mMaybe{u}$ に食わせてMaybe値 $\mMaybe{v}$ ただし
\begin{equation}
  \mMaybe{v}
...
\item 普通の関数 $f\mIn\mProjEXP{\mA }{\mB }$をMaybe値に適用することはできない．関数 $\mFFunc$ をMaybe値に適用するには$f\mMapMaybe\mMaybe{u}$ のようにMaybeマップ演算子が必要であり，この関数適用の結果は $\mMaybeType{b}$ 型である．
...
$\mA $ 型の変数 $\mXVar,y\mIn\mA$ について，関数 $f\mIn\mA\mFuncArrow\mA$ があり
\begin{equation}
  y
  =\mFFunc\mXVar
\end{equation}
...
% ていない変数 $\mXVar$ がふらっと現れ，目の前に $f\mMap x$という式が登場し
% ても，落ち着いて変数 $\mXVar$ の型を調べ，変数 $\mXVar$ がリストならば $\mMap$
% の部分に $\mMapList$ を，変数 $\mXVar$ がMaybeならば $\mMap$ の部分に
...
\begin{equation}
  ...
  =[\mFFunc\mXVar,fy,fz,\mGFunc\mXVar,gy,gz,hx,hy,hz]
\end{equation}
...
\begin{equation}
  \mAppMapList
  \mIn\underbrace{\mA\mapsto\mB }_{[f_0,\mFFunc\dotsb f_n]}
  \mapsto\underbrace{[\mA]}_{[\mXVar_0,\mXVar_1\dotsb \mXVar_n]}
  \mapsto\underbrace{[\mB]}_{[f_0\mXVar_0,f_0\mXVar_1\dotsb f_0\mXVar_n,\mFFunc \mXVar_0,\mFFunc \mXVar_1\dotsb f_n\mXVar_n]}
\end{equation}
と解釈すれば良い．
...
いま関数 $f\mIn\mProjEXP{\mR}{\mA }$ とは別な関数 $g\mIn\mProjEXP{\mA }{\mB }$ があったとしよう．関数 $\mFFunc$ と関数 $\mGFunc$ の合成 $g\mCompFunc f$ の型は
\begin{equation}
...
\begin{equation}
g\mAppMapFunc f=\mLambdaEXP{x}{\mGFunc\mXVar(\mFFunc\mXVar)}
\end{equation}
とする．これは，関数版のピュア演算子の定義と，一般マップ演算子と一般アプリカティブマップ演算子の関係 $f\mMap w=\mPureWith{f}\mAppMap w$ から導かれる．すなわち
\begin{align}
\mConstWith{g}\mAppMapFunc f
&=\mLambdaEXP{x}{\mConstWith{g}x(\mFFunc\mXVar)}\\
&=\mLambdaEXP{x}{(\mLambdaEXP{\_}{g})x(\mFFunc\mXVar)}\\
&=\mLambdaEXP{x}{g(\mFFunc\mXVar)}\\
&=g\mCompFunc f
\end{align}
であるからである．
...
% \left\{
% \begin{array}{ll}
% \mJustWith{\mFFunc\mXVar}&\mIf\mMaybe{u}\equiv\mJustWith{x}\\
% \mNothing&\mOtherwise
% \end{array}\right\}
...
% \end{cases}\\
% &=\begin{cases}
% \mJustWith{\mFFunc\mXVar}&\mIf\left(\mMaybe{u}\equiv\mJustWith{x}\right)\mLogicalAnd\left(\mMaybe{h}\equiv\mJustWith{f}\right)\\
% \mNothing&\mOtherwise
% \end{cases}
% \end{align}
% である．
...
\end{equation}
という風に結果をMaybe変数に保存しなければならない．今まで見てきた $y=\mFFunc\mXVar$ や $\mMaybe{v}=f\mMapMaybe\mMaybe{u}$ の関係とは異なることに注意しよう．
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
ここに $\mXVar$ は非コンテナ変数で，関数 $\mFFunc$ も「普通の」（コンテナに入っていない）関数である．より厳密に言えば $\mXVar\mIn\mA$ かつ $f\mIn\mA\mFuncArrow\mB$ である．従って，関数 $\mVarContainer{i}$ の型は $\mA\mFuncArrow\mPureType{b}$ である．条件 $\mLovelyVar$ には任意の値を入れてよい．
...
\end{equation}
と書ける．うまく行けば $\mVarContainer{a}=\mPureWith{\mFFunc\mXVar}$ となるし，そうでなければ $\mVarContainer{a}=\mPureNothing$ となる．

さて $\mVarContainer{u}=\mPureWith{x},\mVarContainer{v}=\mPureWith{y}$ として，かつ $y=\mFFunc\mXVar$ であるとき
\begin{equation}
...
式\eqref{eq:def-of-i}から $ix=\mPureWith{\mFFunc\mXVar}$ すなわち $i=\mPureWith{f\mAnonParam}$ の関係を抜き出すと式\eqref{eq:i-love-u}は
\begin{equation}
\mVarContainer{a}=\mPureWith{f\mAnonParam}\mBind\mVarContainer{u}
\end{equation}
となる．いま $y=\mFFunc\mXVar$ であったから $\mVarContainer{a}=\mVarContainer{v}$ であり，最終的に
\begin{equation}
\mVarContainer{v}
...
\end{align}
であった．
% 関数ピュア演算子は，任意の引数 $\mXVar$ を関数に置き換える．ただし，その関数は引数を捨てて元の変数 $\mXVar$ を返す．例えば
% \begin{equation}
% x=\mFuncWith{x}y
% \end{equation}
% と，ダミー変数 $\mYVar$ を使って中身の $\mXVar$ を取り出せる．
...
\begin{align}
\mConstWith{x}&=\mLambdaEXP{\_}{x}\\
g\mMapFunc f&=\mLambdaEXP{x}{g(\mFFunc\mXVar)}=g\mCompFunc f\\
g\mAppMapFunc f&=\mLambdaEXP{x}{\mGFunc\mXVar(\mFFunc\mXVar)}\\
g\mBindFunc f&=\mLambdaEXP{x}{g(\mFFunc\mXVar)x}
\end{align}
である．
...
\begin{equation}
  ...
  \right.
\end{equation}
残念ながら，この式は $\mA $ 型の変数 $\mXVar,\mYVar$ の間に等号 $(\equiv)$ が定義されていることが隠れた前提になっているため，正しくない．我々は $\mA $ が型クラス $\mEqTypeClass$ に属すことを要求するので，次のように言い換える．
\begin{multline}
\mInstanceDeclPolymorphic\;
...
\begin{equation}
  ...
  \mWhere\left\{
  \begin{aligned}
    f\mMap{}\mJustWith{x}&=\mJustWith{\mFFunc\mXVar}\\
...
\begin{equation}
  ...
  \left\{
  \begin{aligned}
    f\mMap{}\mRightWith{x}&=\mRightWith{\mFFunc\mXVar}\\
...
\mList{f}&=[f_0,\mFFunc,\dotsb]
\end{align}
の両方をそれぞれ値コンストラクタで包んでアプリカティブマップ演算子を適用すると
\begin{equation}
  \mZipListWith{\mList{f}}\mAppMap\mZipListWith{\mList{x}}
  =[f_0\mXVar_0,\mFFunc \mXVar_1,\dotsb]
\end{equation}
のようにジップ演算子を適用できる．
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    &=f\mXVar\mYVar:\mZipWith f\mList{x}\mList{y}
    \end{aligned}
    \right.
  \end{aligned}
  \right.
...
\begin{align}
  ...
  &=\mLambda x\mLambdaArrow\mJoinList{}(f\mMapList(\mGFunc\mXVar))\\
...
\begin{multline}
  ...
  \begin{aligned}
    ...
    &=\mValueWith{SimpleFunc}{\mLambda\mPairWith{\mXVar}{\mYVar}\mLambdaArrow\mPairWith{\mFFunc\mXVar}{y}}
  \end{aligned}
  \right.
\end{multline}
これがブラケットの正体である．
...
\begin{equation}
  ...
  \mWhere\mSwap\mPairWith{\mXVar}{\mYVar}\mLetEq\mPairWith{y}{x}
\end{equation}
...
ただし $\Vec{\mFFunc}$, $\Vec{\mGFunc}$ はもはや関数ではなく，
$\Vec{\mFFunc}***\Vec{\mGFunc}$ も関数ではない．
...
\begin{equation}
  \mZVar=\mGFunc\mCompFunc \mFFunc x
\end{equation}

\begin{equation}
  \mZVar=\mathop{\mathrm{app}}\mPairWith{\mArrowWith{\mUncurry \mGFunc}\mCompCat\mFirstFunc\mArrowWith{\mFFunc}\mCompCat{}\mArrowWith{\left(\mLambda x\mLambdaArrow\mPairWith{x}{x}\right)}}{x}
\end{equation}
...
\begin{multline}
  ...
  \begin{aligned}
    ...
    \mCircuitWith{\mGFunc}\mCompCat\mCircuitWith{\mFFunc}
    &={}^{\textrm{Circ}}[[\mLambda x\mLambdaArrow(\mGFunc'\mCompCat \mFFunc',z)\\
    &\quad\mWhere{}\left\{
    \mPairWith{\mFFunc'}{y}\mLetEq \mFFunc x;
    \mPairWith{\mGFunc'}{z}\mLetEq \mGFunc y\right\}]]
  \end{aligned}
  \right.
\end{multline}
...
% http://d.hatena.ne.jp/haxis_\mFFunc\mXVar/20110720/1311149995
...
\begin{equation}
\mMainFunc=f_n\mCompFunc f_{n-1}\mCompFunc\dotsb\mCompFunc \mFFunc
\end{equation}
...
\end{aligned}
\end{equation}
とする．ここで $0\le x\le\pi$ の範囲で $\mYVar$ の値を求めたいとしよう．$\mXVar$
の範囲を $n+1$ 分割するとする．
...
\begin{equation}
(f\circledcirc g)t=\sum_x\mFFunc\mXVar*g(t-x)
\end{equation}
...
&\qquad\mList{f}\mLetEq[\mFFunc\mXVar\mListComp x\mFrom[0,1\dotsb n]]\\
...
\begin{equation}
f\circledcirc g=\mLambdaEXP{t}{\sum \mFFunc\mXVar*g(t-x)}
\end{equation}
...
$$
yt=\sum_x\mFFunc\mXVar*g(t-x)
$$

$$
y=\sum_x\mFFunc\mXVar*g(\mAnonParam-x)
$$
...
$\mXVar,y,z$ で表すこととする．このことを数学者は $\mXVar,y,z\in\mRSet$ と書くが，
本書ではこれまで通り
\begin{equation}
x,y,z\mIn\mRSet
\end{equation}
...
ひとつひとつ見ていこう．ここで $\mXVar,y,z\mIn\mRSet$ とする．
\begin{description}
...
呼ぶ．
\item[実数の性質2. 加法の結合性] 任意の $\mXVar,y,z$ について
\begin{equation}
...
である．
\item[実数の性質7. 乗法の結合性] 任意の $\mXVar,y,z$ について
\begin{equation}
(x*y)*\mZVar=x*(y*z)
\end{equation}
である．
...
単位的半群の性質は次の三つである．ただし $\mXVar,y,z$ が集合 $\mSet{M}$ の
元であるとする．
\begin{description}
\item[単位的半群の性質1.] 任意の $\mXVar$ と任意の $\mYVar$ の演算の結果
$\mXVar\mBinOp y$ は $\mSet{M}$ の元である．
\item[単位的半群の性質2. 結合性] 任意の $\mXVar,y,z$ について
\begin{equation}
(x\mBinOp y)\mBinOp \mZVar=x\mBinOp(y\mBinOp z)
\end{equation}
である．
...
&=x\\
\mIfFunc f\mXVar\mYVar&=f\mXVar\mYVar\\
