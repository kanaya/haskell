...
関数 $\hxFunc{f}$ に引数 $\hxVar{x}$を「食わせる」ことを\keyword{関数適用}と呼ぶ．もし $\hxFunc{f}\hxVar{x}$ と書いてあったら，それは $\hxFunc{f}$ と $\hxVar{x}$ の積，つまり $f\times x$ ではなく，従来の $f(x)$ すなわち関数 $\hxFunc{f}$ に引数 $\hxVar{x}$ を与えているものと解釈する．高校生向けの数学書でも $\sin x$ のように三角関数に限ってはカリー風に書くことになっているので，まるで馴染みがないということもないだろう．なお，関数はいつも引数の左側に書くことにする．これを「関数 $\hxFunc{f}$ が変数 $\hxVar{x}$ の\keyword{左から作用する}」と言い，また関数 $\hxFunc{f}$ のことを\keyword{左作用素}とも呼ぶ．
